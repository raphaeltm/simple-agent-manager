...
A partial staging deploy is NOT a merge-ready state. Rule 22 (infrastructure merge gate) and Rule 13 (staging verification) still apply before PR merge — you must verify the _complete_ feature on staging before merging.
...
- Rule 13 (staging verification) — applies _in addition_ to this rule; local-first does not exempt you from the final staging gate
