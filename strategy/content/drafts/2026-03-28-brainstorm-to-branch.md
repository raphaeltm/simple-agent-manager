# Draft: From Brainstorm to Branch

**Status**: First draft
**Target**: SAM blog (`apps/www/src/content/blog/`)
**Category**: devlog
**Estimated length**: ~1,600 words
**Author**: Raph (first person, personal hook from brainstorming sessions with SAM)
...
Task mode is for execution. You describe what needs to happen, the system provisions a workspace, starts an agent, and the agent follows a full development workflow. Branch, implement, test, deploy to staging, open a PR. The task has a lifecycle (queued, in progress, completed, failed) and you can watch it progress or ignore it entirely.
...
It's an MCP tool. An agent running in one workspace can call it to spawn a completely new task. The new task gets its own workspace, its own agent session, its own branch. The parent agent gets back a task ID and keeps going. The child task runs independently, following the full development workflow.

When I click Execute on an idea, or when I tell an agent mid-conversation to "go handle this," this is what happens under the hood. The system provisions a fresh workspace (or grabs a warm one from the pool if available), checks out the repo, generates a task-scoped token so the new agent can access project context via MCP, and starts the agent with instructions to follow the `/do` workflow.

There are guardrails. Tasks can nest up to three levels deep. Each parent can spawn at most five children. A project can have at most ten dispatched tasks running at once. All of these are configurable, but the defaults are there to prevent runaway recursion. (We learned from the 828 Tests incident that you can't just trust agents to know when to stop.)

The important thing is that none of this is prompt chaining. The dispatched agent runs in a real environment with real tools. It compiles code, runs tests, deploys to staging, opens actual pull requests. It's not an LLM generating text about what it would do. It's an agent doing the work.
...
Twenty minutes later, a PR appeared. The dispatched agent had implemented the batch window, added tests, deployed to staging, and verified it worked. I reviewed the PR, merged it, and went back to my conversation.
...
**Things to review:**
- The "concrete example" in the practice section is illustrative, based on the notification batching feature that shipped. If you have a better specific story, swap it in.
- The DAG section is kept short and clearly aspirational, per your direction. Expand or trim based on how much you want to tease.
- The closing is casual per your style. If you want a punchier last line (like the Scaleway post's "Submit a task in French"), add one.
- Title options: "From Brainstorm to Branch" (current), "Don't Break the Flow", "Brainstorm, Dispatch, Keep Going"

**What I avoided:**
- The term "multi-agent" (per your request)
- Marketing language, superlatives, em dashes
- Over-explaining SAM for new readers (brief context, links to intro post)
- Deep architecture details (hinted at, not diagrammed)

**Social media variants to generate once the post is finalized.**
