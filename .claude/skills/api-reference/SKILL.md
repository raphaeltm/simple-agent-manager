...
- `ack_event_delivery` — Idempotently acknowledge an already-acknowledged event delivery, or acknowledge a `queued`/`delivered` delivery when its delivery policy requires acknowledgement. Rejects deliveries whose independent delivery policy does not require acknowledgement, plus `failed` or `expired` terminal deliveries. Subscriptions/routing decide visibility; the separate delivery policy decides whether acknowledgement is required.
...
Event-bus tools reject caller-supplied project/task/session/workspace identity fields; scope comes from the verified MCP token plus current D1 project predicates and an active, unexpired subscription at read/ack time. Event, subscription, and delivery misses intentionally use nondisclosing not-found-or-not-visible errors so agents cannot enumerate cross-project or cross-subscription data.
