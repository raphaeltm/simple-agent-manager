...
- `create_project_event_subscription` / `list_project_event_subscriptions` / `get_project_event_subscription` / `cancel_project_event_subscription` — Task-agent-only ProjectData event-subscription tools. The server derives project, owner, task, workspace, chat session, and agent-session identity from the MCP token; callers cannot supply `projectId`, `owner`, `ownerScope`, or `cancelledBy`. Creates are short-lived and capped by the MCP token lifetime. Requested delivery policy is recorded separately from matching/routing, and this wave resolves non-record-only modes to `recorded_not_injected`; it does not inject prompts, steer runtimes, interrupt runtimes, spawn tasks, or expose human/UI controls. Missing get/cancel requests error by default; `required=false` returns `subscription:null`.
