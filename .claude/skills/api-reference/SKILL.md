...
- `get_resource_history` — Read bounded workspace resource history for a session, task, or workspace through the current project scope. Returns D1 summary/chunk index by default; optional `chunkId` lazy-loads downsampled R2 samples and sanitized tool-span correlation. Tool spans are correlation windows, not causal per-process attribution, and payloads omit prompts, commands, tool arguments/output, paths, environment, and secrets.
...
- `GET /api/admin/project-data/storage/message-upload-quarantine/:projectId` — Superadmin-only bounded inventory of incomplete VM transcript uploads in the root project object, including pending/abandoned status and staged byte counts (`limit` 1–100, `after` JSON tuple returned as `nextCursor`)
