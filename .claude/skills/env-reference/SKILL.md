...
- `PROJECT_DATA_EVENT_BUS_RETENTION_DAYS` — Age before old event-bus events without pending ack-required deliveries may be pruned from ProjectData (default: 14)
- `PROJECT_DATA_EVENT_BUS_RETENTION_BATCH_ROWS` — Maximum old event-bus events pruned per ProjectData alarm (default: 500)
