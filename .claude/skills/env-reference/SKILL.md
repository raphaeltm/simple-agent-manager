...
- `PROJECT_DATA_ARCHIVE_SEARCH_CONCURRENCY` — Maximum archive-owner Durable Object searches executed concurrently within one continuation step (default: `4`)
- `PROJECT_DATA_ARCHIVE_SEARCH_REPAIR_SESSIONS` — Maximum incomplete compact-archive sessions repaired per owner search call (default: `1`)
- `PROJECT_DATA_ARCHIVE_SEARCH_REPAIR_CHUNKS` — Maximum compact R2 chunks consumed by one search-triggered repair step (default: `1`)
- `PROJECT_DATA_ARCHIVE_SEARCH_CONTINUATION_TTL_MS` — Lifetime of a signed project-wide archive-search continuation; continuation does not extend it (default: `900000`)
- `PROJECT_DATA_ARCHIVE_SEARCH_CURSOR_MAX_BYTES` — Maximum encoded byte size accepted or emitted for a signed archive-search continuation (default: `1048576`)
- `PROJECT_DATA_ARCHIVE_SEARCH_ERROR_LIMIT` — Maximum owner/session search errors retained in continuation state and disclosed to callers (default: `20`)
