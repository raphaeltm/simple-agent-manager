...
- `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_BATCH_BYTES` — Hard maximum legacy `tool_metadata` bytes read into JS by one automatic cleanup pass; a fixed exact plan is invalid unless this is at least `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_MAX_ROW_BYTES` (default: `2097152`)
...
- `PROJECT_DATA_TOOL_PAYLOAD_MANUAL_CLEANUP_MAX_BATCH_BYTES` — Hard legacy `tool_metadata` byte cap for one explicit manual cleanup pass (default: `2097152`)
...
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_BATCH_ROWS` — Maximum matching rows examined by one preflight slice (default: `5000`)
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_INTERVAL_MS` — Persisted minimum interval between preflight slices (default: `300000`)
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_BATCHES` — Overall slice ceiling after which an incomplete preflight terminates as truncated (default: `100`)
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_ROWS` — Overall matching-row ceiling for one preflight plan (default: `500000`)
...
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_LEASE_MS` — D1 claim lease that prevents overlapping scheduled slices and must exceed the wall-time budget by at least five seconds (default: `60000`)
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_WALL_TIME_MS` — Per-slice ProjectData RPC timeout; timeouts retain the cursor and fail closed for retry (default: `20000`)
...
- `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_RUN_WALL_TIME_MS` — Aggregate wall-time ceiling for one scheduled preflight invocation; must exceed the per-slice wall-time budget by the return margin (default: `25000`)
