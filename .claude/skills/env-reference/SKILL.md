...
- `DO_SHARD_MIGRATION_THRESHOLD_BYTES` — Primary ProjectData DO storage estimate that starts stopped-session shard migration (default: `7516192768`)
- `DO_SHARD_AGGRESSIVE_THRESHOLD_BYTES` — Storage estimate that drains toward `DO_SHARD_TARGET_SIZE_BYTES` instead of only below the migration threshold (default: `9126805504`)
- `DO_SHARD_HARD_BRAKE_THRESHOLD_BYTES` — Storage estimate that processes full bounded migration batches while eligible stopped sessions remain (default: `9663676416`)
- `DO_SHARD_TARGET_SIZE_BYTES` — Target primary ProjectData DO storage estimate during aggressive migration (default: `5368709120`)
- `DO_SHARD_MAX_SIZE_BYTES` — Estimated migrated payload ceiling for each same-class ProjectData shard DO (default: `2147483648`)
- `DO_SHARD_MIGRATION_BATCH_SIZE` — Max stopped sessions migrated by one ProjectData alarm turn; runtime caps unsafe large overrides (default: `10`)
- `DO_SHARD_CHECK_INTERVAL_MS` — Recheck interval while ProjectData sharding work remains (default: `3600000`)
