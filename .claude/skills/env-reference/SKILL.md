...
- `PROJECT_DATA_ARCHIVE_SHARDING_ENABLED` — Exact opt-in for the external terminal-transcript archive coordinator; missing, malformed, and non-`true` values disable migration (default: `false`)
- `PROJECT_DATA_ARCHIVE_MAX_CANDIDATES_PER_SWEEP` / `PROJECT_DATA_ARCHIVE_MAX_CANDIDATES_HARD_LIMIT` — Candidate journals claimed per sweep and configurable ceiling, additionally capped by the protocol at 25 (defaults: `1` / `25`)
- `PROJECT_DATA_ARCHIVE_MAX_CHUNKS_PER_SWEEP` / `PROJECT_DATA_ARCHIVE_MAX_CHUNKS_HARD_LIMIT` — Idempotent transcript/manifest chunks processed per sweep and configurable ceiling, additionally capped by the protocol at 100 (defaults: `8` / `100`)
- `PROJECT_DATA_ARCHIVE_CHUNK_MAX_ROWS` / `PROJECT_DATA_ARCHIVE_CHUNK_MAX_ROWS_HARD_LIMIT` — Rows per source/target chunk RPC and configurable ceiling, additionally capped by the protocol at 1000 (defaults: `128` / `1000`)
- `PROJECT_DATA_ARCHIVE_CHUNK_MAX_BYTES` — Canonical bytes per chunk RPC, capped below the 32 MiB Durable Object RPC boundary (default: `8388608`)
- `PROJECT_DATA_ARCHIVE_LEASE_MS` — D1 migration-journal lease duration; every reclaim increments the fence epoch (default: `300000`)
- `PROJECT_DATA_ARCHIVE_TERMINAL_GRACE_MS` — Minimum terminal age before a session can become a migration candidate (default: `604800000`)
- `PROJECT_DATA_ARCHIVE_RETRY_BASE_MS` / `PROJECT_DATA_ARCHIVE_RETRY_MAX_MS` — Failed-journal exponential retry bounds (defaults: `300000` / `86400000`)
- `PROJECT_DATA_ARCHIVE_CIRCUIT_FAILURES` / `PROJECT_DATA_ARCHIVE_CIRCUIT_OPEN_MS` — Consecutive project failures that open the circuit and its open duration (defaults: `3` / `3600000`)
- `PROJECT_DATA_ARCHIVE_SHARD_COUNT` / `PROJECT_DATA_ARCHIVE_SHARD_COUNT_HARD_LIMIT` — Deterministic packed owners per project and configurable ceiling, additionally capped by the protocol at 256 (defaults: `16` / `256`)
- `PROJECT_DATA_ARCHIVE_SEARCH_MAX_OWNERS` / `PROJECT_DATA_ARCHIVE_SEARCH_MAX_OWNERS_HARD_LIMIT` — Archive owners queried by project-wide read planes and a non-raiseable request-budget ceiling of 7; overflow is reported as partial (defaults: `7` / `7`)
- `PROJECT_DATA_ARCHIVE_SWEEP_MAX_WALL_MS` — External coordinator wall-clock budget, protocol-capped at 30000 ms (default: `20000`)
- `PROJECT_DATA_ARCHIVE_SWEEP_MAX_IO_OPS` — Conservative external-operation reservations per sweep; work yields before an indivisible stage cannot fit and the non-raiseable protocol cap is 30 (default: `30`)
- `PROJECT_DATA_ARCHIVE_COPY_EXPANSION_RATIO` — Canonical-byte expansion multiplier reserved for SQLite/index growth during placement and rehome; actual target `databaseSize` is rechecked before source deletion (default: `2`)
- `PROJECT_DATA_ARCHIVE_ERROR_MAX_CHARS` — Maximum retained journal error characters, protocol-capped at 10000 (default: `2000`)
- `PROJECT_DATA_ARCHIVE_R2_PREFIX` — Non-expiring private `project-data/...` prefix for immutable canonical recovery chunks and linked sealed-manifest pages (default: `project-data/archive-sharding`)
- `PROJECT_DATA_ARCHIVE_ROOT_COPY_MAX_RATIO` — Maximum root/target `databaseSize` ratio accepted by capacity-gated placement and explicit root copyback (default: `0.7`)
- `PROJECT_DATA_ARCHIVE_ROUTING_VERSION` — Exact Worker routing contract version verified by the pre-deploy compatibility guard (default: `1`)
...
- `PROJECT_DATA_STORAGE_RELIEF_MEASURE_MAX_BATCH_ROWS` — Maximum accepted row budget for superadmin relief measurement slices (default: `5000`)
