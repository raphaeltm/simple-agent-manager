...
work being cut off mid-flight; this one covers work that starts *later* and can
...
4. **Never hand a bounded attempt-timeout context to a long-lived worker.** A
   ...
   silently once it elapses. Derive attempt timeouts *downward* from the resource
...
- **Enter through the real trigger and kill the caller's context.** Start the
  ...
  request returning, *then* trigger the later operation. Calling the long-lived
...
- `.claude/rules/49-capture-prerequisites-before-async-completion.md` — the
  same shape for captured *values* rather than contexts
