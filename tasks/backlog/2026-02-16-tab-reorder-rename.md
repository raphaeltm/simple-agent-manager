...
- [ ] Refactor `workspaceTabs` useMemo to use a single sorted list by `sortOrder` instead of `[...terminal, ...chat]`
- [ ] Assign `sortOrder` to all tabs (terminal and chat) based on creation time
- [ ] Ensure new terminal tabs get `sortOrder = max + 1` (rightmost)
- [ ] Ensure new chat tabs get `sortOrder = max + 1` (rightmost)
- [ ] Persist unified tab order (coordinate between client-side sessionStorage and VM agent SQLite)
- [ ] Update keyboard shortcuts (Ctrl+Tab, Cmd+1-9) to use the new unified order
- [ ] Add unit tests for unified ordering logic

### Phase 2: Tab Rename
- [ ] Add double-click-to-rename to workspace-level tab strip (port from `TabItem.tsx`)
- [ ] Wire terminal tab rename to `multiTerminalRef.current?.renameSession()`
- [ ] Add API endpoint or mechanism for renaming chat session labels
- [ ] Wire chat tab rename to the API
- [ ] Add unit tests for rename behavior (enter edit mode, save, cancel, max length)
- [ ] Verify mobile usability (touch-friendly rename trigger — possibly long-press or context menu)

### Phase 3: Drag-and-Drop Reordering
- [ ] Install `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities`
- [ ] Wrap workspace tab strip with `<DndContext>` + `<SortableContext>`
- [ ] Make each tab a sortable item with `useSortable()`
- [ ] Add `<DragOverlay>` for ghost tab visual
- [ ] Configure `PointerSensor` with `distance: 5` activation constraint
- [ ] Configure `KeyboardSensor` for accessible reordering
- [ ] Add custom screen reader announcements via `DndContext.accessibility`
- [ ] Wire `onDragEnd` to update `sortOrder` for all tabs and persist
- [ ] Add CSS for drag states (opacity, cursor, drop indicator)
...
- [ ] Constitution compliance check (no hardcoded values)
- [ ] Documentation sync (CLAUDE.md recent changes, keyboard shortcuts)
