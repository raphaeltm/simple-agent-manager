...
- `packages/shared/src/types/capacity-pool.ts` exposes `CapacityPoolCandidate` with provider, location, machineSize, runtime, machineClass, and status. It has no provider-native catalog metadata fields such as SKU specs, RAM, disk, price, stale/unavailable state, or arbitrary add-by-SKU mutation payload.
- `packages/shared/src/types/provider.ts` and `apps/api/src/routes/providers.ts` expose the current provider catalog as `locations[]` plus `sizes: Record<VMSize, SizeInfo>`. This can be flattened into provider-native offerings by combining each provider location with each size’s concrete provider type/SKU, vCPU, RAM, disk, and price.
- The current backend defaults service seeds candidate rows from shared provider locations × `small|medium|large`. Until a backend task lands arbitrary provider-native catalog/mutation support, “add offering” in the UI must map to re-activating an existing candidate for the same provider/location/SKU-derived size and should keep the adapter isolated.
