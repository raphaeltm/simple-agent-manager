...
- [ ] Add migration `0094_deployment_custom_domain_lifecycle.sql` with additive
      columns only:
  - [ ] `deployment_environments.desired_routing_revision INTEGER NOT NULL DEFAULT 0`
  - [ ] `deployment_environments.observed_routing_revision INTEGER NOT NULL DEFAULT 0`
  - [ ] `deployment_environments.observed_routing_status TEXT`
  - [ ] `deployment_environments.observed_routing_error TEXT`
  - [ ] `deployment_environments.observed_routing_at TEXT`
  - [ ] `deployment_custom_domains.verified_cname_target TEXT`
  - [ ] `deployment_custom_domains.desired_state TEXT NOT NULL DEFAULT 'active'`
  - [ ] `deployment_custom_domains.routing_status TEXT NOT NULL DEFAULT 'pending_dns'`
  - [ ] `deployment_custom_domains.activation_routing_revision INTEGER`
  - [ ] `deployment_custom_domains.deactivation_routing_revision INTEGER`
  - [ ] `deployment_custom_domains.deleted_at TEXT`
  - [ ] helpful indexes for environment/routing status/desired state.
- [ ] Add `deployment_custom_domain_events` append-only table with nullable
      `node_id` or denormalized node identifier so events survive node deletion.
- [ ] Change `deployment_release_events.node_id` and
...
- [ ] Create a service that increments an environment's desired routing revision
      and records a lifecycle event in one logical operation.
- [ ] Extend custom-domain responses with:
  - [ ] `routingStatus`
  - [ ] `servingStatus`
  - [ ] `desiredState`
  - [ ] `verifiedCnameTarget`
  ...
  - [ ] environment lifecycle/routing context needed by the UI.
- [ ] Ensure `buildVerifiedCustomRouteTargets()` includes only domains that:
  - [ ] are verified,
  - [ ] are not deactivating/deleted,
  - [ ] still match the current expected CNAME target,
  - [ ] have a matching parent route.
- [ ] Add target-change detection so route reorder/target changes reset or
      surface re-verification instead of silently treating stale DNS as active.
- [ ] Add lifecycle event recording for attach, verify success/failure,
...
- [ ] Update `POST /custom-domains` to initialize routing/desired state and
      return the enriched response.
- [ ] Update `POST /:domainId/verify`:
  - [ ] Verify DNS against current expected target.
  - [ ] On success, store `verified_cname_target`, mark routing as activating or
        active depending on observed revision, increment desired routing revision,
        and record events.
  - [ ] On failure, store safe, copyable error detail and avoid routing changes.
- [ ] Change `DELETE /:domainId` from immediate physical delete to "request
      deactivation":
  - [ ] mark desired state deactivating,
  - [ ] increment desired routing revision,
  - [ ] record event,
  - [ ] return 202/enriched domain state instead of `{deleted:true}`.
- [ ] Add any cleanup/finalization helper that can mark deactivated rows
      `deleted_at` / remove them only after observed node revision confirms the
      route was dropped.
- [ ] Update `GET /custom-domains` to keep preserved domains visible even when
      routes are inactive/missing.
- [ ] Update `GET /public-routes` to derive from the latest stored release even
...
- [ ] Extend API heartbeat parsing to accept observed route config state:
      `routingRevision`, `routingStatus`, `routingError`.
- [ ] When desired routing revision exceeds observed routing revision, advertise
      `pendingRouteConfigs: [{ environmentId, revision }]` only when:
  - [ ] environment is active/starting,
  - [ ] an app release is currently applied,
  - [ ] volume readiness gates are satisfied for volume environments.
- [ ] Add `GET /api/nodes/:id/deploy-routes?revision=R&environmentId=E`
      as a callback-JWT route mounted before session-auth node routes.
- [ ] The route-config payload must be signed with the deploy signing key and
      include environment id, node id, current applied release seq, routing
      revision, expiry, and route targets.
- [ ] Add route-order/auth tests proving VM-agent callback JWT auth works and
      session-auth middleware cannot intercept the new callback route.
- [ ] Add D1/route tests proving verify/delete queue pending route configs.

### Phase E — VM-agent route-only reconciliation

- [ ] Add Go route-config payload/signature types and verifier methods separate
      from application release `ApplyPayload` replay rules.
- [ ] Extend heartbeat response parsing with `pendingRouteConfigs`.
- [ ] Add `FetchAndApplyRoutes(ctx, revision)`:
  - [ ] fetch signed route-config payload,
  - [ ] verify environment/node/current release seq/revision/expiry/signature,
  - [ ] generate Caddy snippet from routes,
  - [ ] write the current release's Caddyfile/snippet,
  - [ ] reload Caddy,
  - [ ] update current release metadata with routing revision/status,
  - [ ] update observed heartbeat state.
- [ ] Ensure route-only apply does not run `docker compose pull/up/down` and does
      not change `appliedSeq`.
- [ ] Add Go tests for success, signature replay/revision mismatch, no-current
      release, Caddy reload failure, and "no compose commands invoked."

### Phase F — UI behavior

- [ ] Replace the "active on next deployment apply" copy with explicit DNS vs
      routing vs serving state.
- [ ] Keep saved domains visible while environment status is stopped, starting,
      or error.
- [ ] Show inactive/stopped, start failed, route missing, activating,
      active, deactivating, DNS mismatch, and target-changed/recheck states.
- [ ] Adjust delete UI to show "deactivation requested" until observed rather
      than immediately removing the row.
- [ ] Group domains by stable parent service/port when routeIndex changes, and
      show a warning if the expected CNAME target changed.
- [ ] Use TanStack Query or otherwise preserve stale-while-revalidate behavior
      without unmounting visible content on refetch.
- [ ] Keep mobile layout at 375px free of horizontal overflow.

### Phase G — Tests and verification

- [ ] API unit/integration tests for enriched custom-domain states, route target
      changes, delete/deactivation behavior, public-routes while stopped/error,
      lifecycle events, and routing revision updates.
- [ ] Heartbeat vertical-slice tests for verify → pending route config →
      observed active, and delete → pending route config → deactivated.
- [ ] Go tests for route-only apply and route-config signature contract.
- [ ] Cross-boundary tests proving TypeScript route-config payload shape matches
      Go decoding/signature verification.
- [ ] Frontend component tests for state mapping and delete/deactivation UI.
- [ ] Playwright visual audit for Domains tab with normal data, stopped/error
      preserved domains, long hostnames, many domains, route-missing, and error
      states at 375x667 and 1280x800.
- [ ] Full local gates: lint, typecheck, test, build, migration safety.
- [ ] Specialist reviews: task-completion-validator, cloudflare-specialist,
