...
- [ ] Add focused unit tests for provider-mode resolution, no-silent-fallback behavior, frontend availability, quota ceiling validation, and cost-cap enforcement. (Provider-mode/no-silent-fallback/frontend availability covered in this pass; quota ceiling/cost-cap tests remain with the unchecked quota work.)
...
- [ ] Run specialist validation: env-validator, constitution-validator, security-auditor, doc-sync-validator, plus task-completion-validator and relevant UI/API reviewers. (UI and security checklists reviewed for this scoped provider-selection pass; full quota/admin-cap validation remains with the unchecked quota work.)
...
- [ ] User sees free-tier AI allowance and current usage before/after enabling SAM provider. (Allowance context is present; live current-usage display remains with the quota/budget work.)
