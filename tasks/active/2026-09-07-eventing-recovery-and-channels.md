...
E1 owns new channel files/types/tests plus narrow canonical admission hooks and member subscription route adapters. Additive DO migration **050** reserved for E1; do not duplicate or replace A3 DO048 or C2 DO049. Do not modify generic outbox, credential ingestion, recovery/task-runner interfaces, schedules/watches or frontend files. Coordinate narrow overlapping index/barrel/admission changes explicitly. D schedules will use DO051+ after E1 schema is available; migration numbering gaps must not crash migration-order checks in isolated child validation, and final integration must be contiguous.
...
Fresh E1 now wires the five MCP tools and member catalog/history REST routes to canonical ProjectData events and subscriptions. Schema050 is still unreleased and now adds indexed channel generation/sequence fields and nullable subscription catch-up checkpoint fields. Catalog counts are explicitly lifetime-only. No deletion trigger, secondary event queue, or cascading checkpoint table is added. Catch-up validates consecutive sequence numbers and the snapshot tail before committing a page; history discloses retention gaps. Live filters use stable channel names; history cursors use generation IDs. Empty catalog reclamation excludes retained events and active unfinished catch-up, so it never breaks live name-based subscriptions.
...
- Mailbox capacity uses one bounded active-only indexed query in ordinary admission and event wake preflight. Additive DO migration055 installs the matching partial index; real workerd retained-history/rows-read proof pending.
...
Evidence at this checkpoint: first corrected recovery/schedule run96/96 pass; seven new cases fail against saved pre-fix implementation (restored afterward). Reserved creator tests29 pass, and removing guard fails five denial scenarios while three valid-role controls pass. New schedule admission-bound tests5/5 pass. D1/DO migration safety pass before additive055. After consolidating source guards, one fixture required explicitly active users; final verification pending. File-size gate required extraction of materialization batch storage and reserved D1 submission storage, plus removing duplicate recovery guard.
