...
- PR CI policy checks caught an 801-line env schema file after the new optional field and missing structured PR evidence markers; the whitespace-only overflow was removed, the exact file-size gate now passes, and the required preflight/specialist evidence format is populated.
