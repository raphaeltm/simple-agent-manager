...
- Recovery rerun at `23755768a`: the focused API slice passed 56/56 tests and the focused web slice passed 32/32 tests. Temporarily restoring the old ungated predicate made the production-shaped handler-boundary case fail with `processed: false`, `matchedTriggers: 0`, and `comment does not start with '/sam'`; restoring the explicit `issue_comment` guard made the same case pass.
- Recovery Playwright rerun: after installing Chromium and its system dependencies in the lightweight workspace, the focused audit passed 8/8 executed cases with 8 expected cross-project skips. In addition to overflow/clipping checks, list cards now assert measured title/source/action/card relationships, the trigger detail asserts measured label/value/row containment, and both form modes assert measured header/body/footer/focused-control relationships at 375x667 and 1280x800.
...
- Recovery screenshot self-review: opened all eight regenerated PNGs for the list, issues detail, issue-comment form, and issues form at both required viewports. Long names/descriptions wrap or intentionally clamp inside their cards; the stale issues prefix is absent while the issue-comment `/sam` prefix remains visible; the detail row reads `None`; drawer chrome stays fixed while the focused stress input remains within the scroll body. No horizontal overflow, unintended clipping, overlap, or unreadable responsive layout remained.
