...
5. **Workspace card two-row layout.** At 375px a running workspace's row holds status pill + title + branch + Open + menu; with both a long title and long branch, truncation is inherent even after the fixes here (title priority + 64px branch floor). The full solution is stacking title and branch on separate rows on narrow viewports — a layout change worth a deliberate pass.
6. **Deployment "Destroy Env" prominence.** A filled red button at the top of the environment page is the loudest element on the screen. It matches the house danger style (which F4 now makes consistent), so I left it — but if destructive-action demotion is ever wanted, this is the first candidate.
...
- An adversarial **ui-ux specialist** review of the full diff returned an initial FAIL with four substantive findings, all addressed in commit 204a0bdc1: (1) the crash-guard sweep missed the *sibling* catalog consumers — `useProviderCatalog` (crashes TaskSubmitForm, Node, and ProjectSettings on a malformed payload) and `ProfileFormDialog` — both now guarded; (2) two remaining bare `setUnreadCount` call sites in `useNotifications` (dismiss refetch, WS `unread_count` frame) now guarded; (3) `TruncatedSummary` floats over the same scroller as the session header with the most transparent background of the three stacked elements — same scrim applied; (4) the WorkspaceCard worst case (long title + long branch + running) still degraded both labels — the branch now has a `min-w-16` floor so it stays identifiable, and the sweep fixture exercises exactly that combination. Its remaining note — that a 375px row with status pill + title + branch + Open + menu is inherently tight and a stacked two-row layout would be the full solution — is recorded under "Proposed but intentionally not implemented".
