...
- [ ] Add handler-level task ownership preflight before invoking `overrideTaskState`.
- [ ] Log cross-project rejection with caller `projectId`, target `taskId`, target `projectId`, expected/received IDs, and action.
- [ ] Add Durable Object guard that mirrors `cancelMission` by verifying the mission is tracked before override.
- [ ] Add Durable Object task ownership check before writing state.
- [ ] Add `project_id = ?` to the final `UPDATE tasks` predicate.
- [ ] Add positive same-project override regression coverage.
- [ ] Add cross-project IDOR regression coverage proving project B task state remains unchanged.
- [ ] Add handler defence-in-depth unit coverage where a mismatched task row is returned and the handler rejects.
- [ ] Run local quality gates for impacted API code.
- [ ] Run local specialist review and address findings.
