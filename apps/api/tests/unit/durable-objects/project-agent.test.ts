...
const PROJECT_AGENT_TOOLS_IMPORT_TIMEOUT_MS = 30_000;
...
describe('Project Agent Tool Definitions', () => {
  it(
    ...
    async () => {
      const { PROJECT_AGENT_TOOLS } =
        await import('../../../src/durable-objects/project-agent/tools');
  ...
  it(
    'includes expected tool categories',
    async () => {
      const { PROJECT_AGENT_TOOLS } =
        await import('../../../src/durable-objects/project-agent/tools');
  ...
  it(
    'does NOT include cross-project tools',
    async () => {
      const { PROJECT_AGENT_TOOLS } =
        await import('../../../src/durable-objects/project-agent/tools');
      const names = PROJECT_AGENT_TOOLS.map((t) => t.name);
...
describe('Project Agent Tool Execution', () => {
  ...
  it(
    'injects projectId from context for project-scoped tools',
    async () => {
      const { executeProjectTool } =
        await import('../../../src/durable-objects/project-agent/tools');
  ...
  it(
    'returns error when projectId is missing from context',
    async () => {
      const { executeProjectTool } =
        await import('../../../src/durable-objects/project-agent/tools');
  ...
  it(
    'does not inject projectId for tools that do not need it',
    async () => {
      const { executeProjectTool } =
        await import('../../../src/durable-objects/project-agent/tools');
...
describe('Project Agent System Prompt', () => {
  it('exports a system prompt focused on project scope', async () => {
    const { PROJECT_AGENT_SYSTEM_PROMPT } =
      await import('../../../src/durable-objects/project-agent/system-prompt');
