...
import { DefaultVmPromptDeliveryAdapter, type VmPromptDeliveryAdapter } from '../../../src/services/vm-prompt-delivery-adapter';
import { acceptedPromptResponse } from '../../helpers/vm-prompt-delivery-fixtures';
import { createSqlStorage } from './sql-storage-test-utils';

const preparationMocks = vi.hoisted(() => ({ recover: vi.fn(), capabilities: vi.fn(), send: vi.fn() }));
vi.mock('../../../src/services/session-recovery', () => ({ ensureSessionRecovery: preparationMocks.recover }));
...
describe('ProjectData durable prompt delivery', () => {
  ...
  it('keeps a slow recovery retryable and sends once after the late preparation finishes', async () => {
    ...
    preparationMocks.recover.mockImplementation(() => new Promise((resolve) => { finishRecovery = resolve; }));
    preparationMocks.capabilities.mockResolvedValue(capabilities);
    preparationMocks.send.mockImplementation(async () => acceptedPromptResponse('acp-1', 'delivery-1', 'runtime-1', Date.now()));
    const target = {
      workspace_id: 'workspace-1', user_id: 'user-1', workspace_status: 'sleeping',
      node_id: 'node-1', node_status: 'running', node_health_status: 'healthy',
      node_runtime: 'vm', agent_session_id: 'acp-1', agent_session_status: 'running',
    };
    const env = { DATABASE: { prepare: () => ({ bind: () => ({ first: async () => target }) }) } } as never;
    ...
    expect(mailbox.getMessage(sql, 'delivery-1')).toMatchObject({
      deliveryState: 'retry_wait', terminalReason: null, acceptedAt: null,
    });
    ...
    await expect(runPromptDeliveryClaim(sql, env, config, retry, adapter, hooks)).resolves.toMatchObject({ kind: 'accepted' });
    expect(mailbox.getMessage(sql, 'delivery-1')).toMatchObject({ deliveryState: 'acked', terminalReason: null });
  ...
  it('never submits from a capability preparation that completes after its deadline', async () => {
    ...
    preparationMocks.capabilities.mockImplementation(() => new Promise((resolve) => { finishProbe = resolve; }));
    const env = {} as never;
    const adapter = new DefaultVmPromptDeliveryAdapter(env);
    accept();
    const claim = claimDuePromptDeliveries(sql, config)[0]!;
    const pending = adapter.submit({
      projectId: 'project-1', claim, allowLegacyVm: false, requestTimeoutMs: config.backgroundTimeoutMs,
      resolvedTarget: {
        projectId: 'project-1', chatSessionId: 'chat-1', workspaceId: 'workspace-1', nodeId: 'node-1',
        agentSessionId: 'acp-1', userId: 'user-1', runtimeIdentity: 'runtime-1', runtime: 'vm',
      },
    });
    await vi.advanceTimersByTimeAsync(config.backgroundTimeoutMs);
  ...
  it('retries a recovery exception before any prompt could have been sent', async () => {
    preparationMocks.recover.mockRejectedValue(new Error('Recovery storage temporarily unavailable'));
    const env = { DATABASE: { prepare: () => ({ bind: () => ({ first: async () => null }) }) } } as never;
    accept();
    const claim = claimDuePromptDeliveries(sql, config)[0]!;
    await expect(runPromptDeliveryClaim(sql, env, config, claim, new DefaultVmPromptDeliveryAdapter(env), hooks))
      .resolves.toMatchObject({ kind: 'retry', reason: 'not_ready' });
    expect(mailbox.getMessage(sql, 'delivery-1')).toMatchObject({ deliveryState: 'retry_wait', terminalReason: null });
    expect(preparationMocks.send).not.toHaveBeenCalled();
  });
