...
describe('task submit — configurable MAX_TASK_MESSAGE_LENGTH', () => {
  ...
  it('falls back to default when env var is absent', () => {
    ...
    expect(submitSource).toContain(
      'parsePositiveInt(c.env.MAX_TASK_MESSAGE_LENGTH, DEFAULT_MAX_MESSAGE_LENGTH)'
    );
  });
...
describe('canonical placement — workspace count limit', () => {
  const doSource = [
    'index.ts',
    'types.ts',
    'node-steps.ts',
  ...
  const placementSource = [
    readFileSync(resolve(process.cwd(), 'src/services/node-selector.ts'), 'utf8'),
    readFileSync(resolve(process.cwd(), 'src/services/placement-explanation.ts'), 'utf8'),
  ].join('\n');

  it('references MAX_WORKSPACES_PER_NODE env var', () => {
    expect(placementSource).toContain('MAX_WORKSPACES_PER_NODE');
  });

  it('references DEFAULT_MAX_WORKSPACES_PER_NODE constant', () => {
    expect(placementSource).toContain('DEFAULT_MAX_WORKSPACES_PER_NODE');
  });
  ...
  it('queries workspace count per node for limit enforcement', () => {
    expect(placementSource).toContain('node.activeWorkspaceCount >= request.maxWorkspacesPerNode');
    expect(doSource).toContain('selectNodeWithExplanation');
  });
});
