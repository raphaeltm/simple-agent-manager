...
describe('resolveDebugAgentConfig', () => {
  ...
  it('falls back safely when a numeric override is invalid', () => {
    expect(resolveDebugAgentConfig({ DEBUG_AGENT_MAX_TURNS: 'invalid' } as Env).maxTurns)
      .toBe(DEFAULT_DEBUG_AGENT_MAX_TURNS);
  });
  it('rejects models outside the registered Workers AI tool-call set', () => {
    expect(() => resolveDebugAgentConfig({
      DEBUG_AGENT_MODEL: 'claude-sonnet-5',
    } as Env)).toThrow('registered Workers AI model with tool support');
  });
});
