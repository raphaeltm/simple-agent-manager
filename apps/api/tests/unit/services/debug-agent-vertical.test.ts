import { DEFAULT_DEBUG_AGENT_DAILY_TOKEN_LIMIT } from '@simple-agent-manager/shared';
...
describe('runDebugDiagnosis vertical slice', () => {
  ...
  it('allows automated diagnosis to continue past the old two-turn token ceiling', async () => {
    const main = new Database(':memory:');
    main.exec(`
      CREATE TABLE debug_diagnoses (
        id TEXT PRIMARY KEY, error_id TEXT, start_time TEXT NOT NULL, end_time TEXT NOT NULL,
    ...
    `);
    const timestamp = Date.parse('2026-08-26T10:00:00.000Z');
    observability
      .prepare(
        `INSERT INTO platform_errors
          (id, source, level, message, stack, context, user_id, node_id, workspace_id,
           ip_address, user_agent, timestamp, created_at)
         VALUES ('err-1', 'api', 'error', 'provider failed', NULL, '{}', NULL, NULL, NULL,
           NULL, NULL, ?, ?)`
      )
    ...
    const responses = [
      {
        ...
        usage: { prompt_tokens: 8000, completion_tokens: 500, total_tokens: 8500 },
      },
      {
        choices: [
          {
      ...
      },
      {
        choices: [
          {
            message: {
              content: '## Summary\nThe diagnosis needed a third model turn.',
              tool_calls: [],
            },
          },
        ],
    ...
    expect(diagnosis.diagnosis).toContain('third model turn');
    expect(diagnosis.usage).toMatchObject({
      turns: 3,
      inputTokens: 24000,
      outputTokens: 1500,
      totalTokens: 25500,
      dailyTokensUsed: 25500,
