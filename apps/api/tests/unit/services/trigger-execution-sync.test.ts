...
function createMockDb(
  ...
  const db = {
    ...
    _internal: {
      bindFn: ReturnType<typeof vi.fn>;
      firstFn: ReturnType<typeof vi.fn>;
...
describe('syncTriggerExecutionStatus', () => {
  ...
  it('syncs to completed when task has a trigger execution', async () => {
    ...
    expect(db.prepare).toHaveBeenCalledWith('SELECT trigger_execution_id FROM tasks WHERE id = ?');

    // Should have updated trigger_executions with idempotent guard
    expect(db.prepare).toHaveBeenCalledWith(expect.stringContaining("status = 'running'"));
  });
  ...
  it('does not throw when SELECT fails', async () => {
    ...
    // Should not throw
    await expect(syncTriggerExecutionStatus(db, 'task-5', 'completed')).resolves.toBeUndefined();
  });
  ...
  it('allows hard-residence backstop failures to be reconciled by final task status', async () => {
    const db = createMockDb({ triggerExecutionId: 'exec-hard-max' });

    await syncTriggerExecutionStatus(db, 'task-9', 'completed');

    expect(db.prepare).toHaveBeenCalledWith(
      expect.stringContaining("status = 'failed' AND error_message LIKE ?")
    );
    const updateCall = db._internal.bindFn.mock.calls[1];
    expect(updateCall[0]).toBe('completed');
    expect(updateCall[2]).toBeNull();
    expect(updateCall[3]).toBe('exec-hard-max');
    expect(updateCall[4]).toBe('Trigger execution exceeded hard maximum residence%');
  });
});
