...
describe('feature token budget', () => {
  it('uses an isolated deployment feature key in the KV fallback', async () => {
    const kv = createKv();
    expect(buildFeatureBudgetKey('deployment-debug-agent', new Date('2026-07-29T12:00:00Z'))).toBe(
      'ai-feature-budget:deployment-debug-agent:2026-07-29'
    );
  ...
  it('releases unused reservations without crossing below zero', async () => {
    ...
    expect(await releaseFeatureTokenBudget(kv, 'deployment-debug-agent', 30, 100)).toMatchObject({
      allowed: true,
      usedTokens: 50,
    });
    expect(await releaseFeatureTokenBudget(kv, 'deployment-debug-agent', 100, 100)).toMatchObject({
      allowed: true,
      usedTokens: 0,
    });
  });
  ...
  it('delegates concurrent enforcement to one feature-scoped Durable Object', async () => {
    ...
    expect((await getFeatureTokenBudget(kv, 'deployment-debug-agent', 100, env)).usedTokens).toBe(
      100
    );
