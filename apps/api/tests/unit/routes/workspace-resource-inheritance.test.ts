...
describe('direct workspace resource inheritance through HTTP and persisted reservations', () => {
  it.each([
    ...
    {
      label: 'partial',
      body: { resourceRequirements: { minMemoryGb: 2 } },
      cpu: 1000,
      memory: 2048,
      disk: 2048,
      source: 'project',
    ...
    {
      label: 'fractional workload',
      body: { resourceRequirements: { minVcpu: 0.5, minMemoryGb: 1.5, minDiskGb: 0.25 } },
      cpu: 500,
      memory: 1536,
      disk: 256,
      source: 'task',
    ...
    async ({ body, cpu, memory, disk, source }) => {
      ...
      const workspace = f.sqlite
        .prepare('SELECT resolved_reservation_json FROM workspaces')
        .get() as { resolved_reservation_json: string };
      ...
      const task = f.sqlite
        .prepare("SELECT resolved_reservation_json FROM tasks WHERE title = 'Inherited workspace'")
        .get();
