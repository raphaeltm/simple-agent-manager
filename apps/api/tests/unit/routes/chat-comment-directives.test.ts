...
type ProjectDataCommentTestStub = {
  ...
  getMessageCommentThread: ReturnType<typeof vi.fn>;
  recordMessageCommentDirectiveDelivery: ReturnType<typeof vi.fn>;
...
describe('chat comment directive route', () => {
  ...
  it('queues one comment directive for the authenticated project session', async () => {
    ...
    expect(env._projectDataStub.getMessageCommentThread).toHaveBeenCalledWith({
      sessionId: 'session-1',
      threadId: 'thread-1',
    });
