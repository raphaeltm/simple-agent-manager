...
describe('MCP update_trigger and delete_trigger handlers', () => {
  ...
  it('updates modern resource requirements while preserving explicit false', async () => {
    insertTrigger('trigger-resources');

    const response = await handleUpdateTrigger(
      'req-1',
      {
        triggerId: 'trigger-resources',
        resourceRequirements: { minVcpu: 2, minDiskGb: 80, exclusiveNode: false },
      },
      tokenData,
      env
    );
    ...
    const row = sqlite
      .prepare('SELECT * FROM triggers WHERE id = ?')
      .get('trigger-resources') as TriggerRow;

    expect(row.resource_requirements_json).toBe(
      '{"minVcpu":2,"minDiskGb":80,"exclusiveNode":false}'
    );
    expect(payload.resourceRequirementsJson).toBe(row.resource_requirements_json);
  });

  it('clears resource requirements with explicit null', async () => {
    insertTrigger('trigger-clear');
    sqlite
      .prepare('UPDATE triggers SET resource_requirements_json = ? WHERE id = ?')
      .run('{"minVcpu":2}', 'trigger-clear');

    const response = await handleUpdateTrigger(
      'req-1',
      { triggerId: 'trigger-clear', resourceRequirements: null },
      tokenData,
      env
    );
    const payload = parseContent(response);
    const row = sqlite
      .prepare('SELECT * FROM triggers WHERE id = ?')
      .get('trigger-clear') as TriggerRow;

    expect(row.resource_requirements_json).toBeNull();
    expect(payload.resourceRequirementsJson).toBeNull();
  });

  it('rejects malformed resource requirements without mutating the trigger', async () => {
    insertTrigger('trigger-invalid');

    const response = await handleUpdateTrigger(
      'req-1',
      { triggerId: 'trigger-invalid', resourceRequirements: { exclusiveNode: 'false' } },
      tokenData,
      env
    );
    const row = sqlite
      .prepare('SELECT * FROM triggers WHERE id = ?')
      .get('trigger-invalid') as TriggerRow;
