...
async function seedRun(runId: string, userId: string): Promise<void> {
  ...
  ).bind(runId, now, now, now, new Date(Date.now() + 60_000).toISOString(), userId, now, now).run();
}
...
describe('DiagnosisRunner DO durability and fault injection', () => {
  ...
  it('fault-injected restart after an external-step claim fails visibly without repeating the step', async () => {
    ...
    ).bind(runId).first<{ status: string; error_message: string }>();
    ...
    ).bind(runId).all<{ error_code: string }>();
    expect(events.results).toEqual(expect.arrayContaining([{ error_code: 'AMBIGUOUS_STEP_OUTCOME' }]));
  });
  ...
  it('cancellation is idempotent even before the runner alarm executes', async () => {
    ...
    ).bind(runId).first<{ status: string; cancel_requested_at: string }>();
