...
import {
  isMailboxAtCapacity,
  MAILBOX_CAPACITY_QUERY,
} from '../../src/durable-objects/project-data/mailbox-capacity';
    ...
    await runInDurableObject(stub, (_instance, state) => {
      ...
      sql.exec(`INSERT INTO chat_sessions (id, task_id, status, message_count, started_at, created_at, updated_at)
        VALUES ('target', 'task', 'active', 0, 1000, 1000, 1000),
          ('old-chat', NULL, 'active', 0, 1000, 1000, 1000),
          ('another-chat', NULL, 'active', 0, 1000, 1000, 1000)`);
      ...
      expect(
        plan.some((row) => String(row.detail).includes('idx_session_inbox_active_capacity'))
      ).toBe(true);
      ...
      expect(
        isTargetAtWakeCapacity(sql, { MAILBOX_MAX_MESSAGES_PER_PROJECT: '2' } as Env, 'target')
      ).toBe(true);
      expect(() =>
        enqueueMessage(sql, {
          id: 'overflow',
      ...
      expect(isMailboxAtCapacity(sql, 2)).toBe(false);
      expect(
        sql.exec("SELECT content FROM session_inbox WHERE id = 'history-1'").toArray()
      ).toEqual([{ content: 'Retained message text' }]);
    });
  });
});
