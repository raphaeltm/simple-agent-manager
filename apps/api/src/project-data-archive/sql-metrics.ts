 ...
 */
export async function measureArchiveSql<T>(
  sql: SqlStorage,
  sessionId: string,
  phase: string,
  operation: (measured: SqlStorage) => Promise<T>
): Promise<T> {
  let rowsWritten = 0;
  ...
  try {
    return await operation(measured);
  } finally {
    log.info('project_data_archive_sql_usage', {
      sessionId,
      phase,
      rowsWritten,
      databaseSizeBeforeBytes: before,
      databaseSizeAfterBytes: sql.databaseSize,
    });
  }
}
