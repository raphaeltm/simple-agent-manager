...
const ChatSessionListRowSchema = v.object({
  ...
  // Last real (non-system) message time — the sidebar ordering key. Optional
  // because a row read mid-migration (or a legacy projection) may not carry it;
  // `updated_at` is the fallback, matching the SQL-side COALESCE.
