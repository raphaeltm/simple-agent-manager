...
export function createMissionStateEntry(
  ...
  const count = parseCountCnt(
    sql
      .exec('SELECT COUNT(*) as cnt FROM mission_state_entries WHERE mission_id = ?', missionId)
      .toArray()[0],
    'mission_state_entry_count'
  );
  if (count >= limits.maxStateEntries) {
    throw new Error(`Maximum state entries per mission (${limits.maxStateEntries}) reached`);
  }

  if (title.length > limits.stateTitleMaxLength) {
    throw new Error(`Title exceeds maximum length (${limits.stateTitleMaxLength})`);
  }
  if (content && content.length > limits.stateContentMaxLength) {
    throw new Error(`Content exceeds maximum length (${limits.stateContentMaxLength})`);
  }
...
export function getMissionStateEntry(sql: SqlStorage, entryId: string) {
  const rows = sql.exec('SELECT * FROM mission_state_entries WHERE id = ?', entryId).toArray();
...
export function deleteMissionStateEntry(sql: SqlStorage, entryId: string): boolean {
  const existing = sql.exec('SELECT id FROM mission_state_entries WHERE id = ?', entryId).toArray();
...
export function createHandoffPacket(
  ...
  const count = parseCountCnt(
    sql
      .exec('SELECT COUNT(*) as cnt FROM handoff_packets WHERE mission_id = ?', missionId)
      .toArray()[0],
    'handoff_packet_count'
  );
  if (count >= limits.maxHandoffs) {
    throw new Error(`Maximum handoff packets per mission (${limits.maxHandoffs}) reached`);
  }

  if (summary.length > limits.summaryMaxLength) {
    throw new Error(`Summary exceeds maximum length (${limits.summaryMaxLength})`);
  }
  ...
  if (openQuestions.length > limits.maxOpenQuestions) {
    throw new Error(`Maximum open questions per handoff (${limits.maxOpenQuestions}) exceeded`);
  }
  if (artifactRefs.length > limits.maxArtifactRefs) {
    throw new Error(`Maximum artifact refs per handoff (${limits.maxArtifactRefs}) exceeded`);
  }
...
export function getHandoffPackets(sql: SqlStorage, missionId: string) {
  return sql
    .exec(`SELECT * FROM handoff_packets WHERE mission_id = ? ORDER BY created_at DESC`, missionId)
    .toArray()
    .map(parseHandoffPacketRow);
}

export function getHandoffPacket(sql: SqlStorage, handoffId: string) {
  const rows = sql.exec('SELECT * FROM handoff_packets WHERE id = ?', handoffId).toArray();
  if (rows.length === 0) return null;
