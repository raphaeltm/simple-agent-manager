...
function hasRetryCapacity(sql: SqlStorage, env: Env, schedule: ProjectSchedule): boolean {
  ...
  const active = sql.exec(
    ...
    schedule.projectId, limits.maxSchedules
  ).toArray();
  if (active.length >= limits.maxSchedules) return false;
  if (!schedule.watchId) return true;
  const watch = sql.exec('SELECT max_concurrent FROM project_standing_watches WHERE project_id=? AND id=?',
    schedule.projectId, schedule.watchId).toArray()[0];
  ...
  const live = sql.exec(
    ...
    schedule.watchId, schedule.id, watch.max_concurrent
  ).toArray();
...
export async function withScheduleExecution(
  ...
  const reservations = new Map(schedules.map((schedule) => [schedule.id, sql.exec(
    `SELECT reserved_task_id, reserved_session_id, submission_completed_at
     FROM project_schedules WHERE project_id = ? AND id = ?`,
    schedule.projectId, schedule.id
  ).toArray()[0]]));
  const taskIds = [...new Set(schedules.flatMap((schedule) => {
  ...
  for (const ids of chunkIdsForBindBudget(taskIds, 1)) {
    try {
      ...
      ).bind(schedules[0]!.projectId, ...ids).all<TaskReceipt>();
  ...
  return schedules.map((schedule) => {
    ...
    const taskId = typeof reservation?.reserved_task_id === 'string'
      ? reservation.reserved_task_id : schedule.resultTaskId;
    const sessionId = typeof reservation?.reserved_session_id === 'string'
      ? reservation.reserved_session_id : schedule.resultSessionId;
    const execution: ProjectScheduleExecution = {
      ...
      submissionDeadline: schedule.action.kind === 'start_session'
        ? Math.min(schedule.expiresAt, schedule.dueAt + scheduleLimits(env).maxDeferralMs) : null,
    };
    if (schedule.action.kind === 'message_session' && schedule.deliveryId) {
      const receipt = sql.exec(
        `SELECT delivery_state, last_error FROM session_inbox WHERE id = ? AND target_session_id = ?`,
        schedule.deliveryId, schedule.action.sessionId
      ).toArray()[0];
      ...
      const matches = task && task.source_execution_id === schedule.id &&
        task.checkpoint_session_id === sessionId && task.chat_session_id === sessionId &&
        task.task_user_id === schedule.creatorUserId && task.checkpoint_user_id === schedule.creatorUserId &&
      ...
      execution.retrySubmissionAllowed = !unavailable && Boolean(schedule.eventId) &&
        ['failed', 'ambiguous'].includes(schedule.state) && schedule.nextAttemptAt === null &&
        reservation?.submission_completed_at === null && execution.submissionDeadline! > now &&
        Boolean(matches && task.status === 'queued') && hasRetryCapacity(sql, env, schedule);
    }
...
export async function reconcileSchedule(
  ...
  if (!isPlainObject(request) || Object.keys(request).some((key) =>
    !['expectedVersion', 'retrySubmission'].includes(key))) {
  ...
  if (schedule.version !== version) throw new ProjectEventValidationError('Schedule version conflict');
  ...
  if (request.retrySubmission === true) {
    if (!execution.retrySubmissionAllowed) {
      throw new ProjectEventValidationError('Task submission retry is unavailable; inspect the receipt and original deadline');
    }
  ...
  if (!current || current.version !== version || current.state !== schedule.state ||
  ...
  const terminal = schedule.action.kind === 'message_session'
    ? ['acked', 'expired', 'failed'].includes(execution.status)
    : ['completed', 'failed', 'cancelled'].includes(execution.status);
  const started = schedule.action.kind === 'start_session' &&
    ['delegated', 'in_progress', 'awaiting_followup', 'completed', 'failed', 'cancelled'].includes(execution.status);
  const known = !['unavailable', 'not_started', 'ambiguous'].includes(execution.status);
  const outcome = retry ? 'retry_scheduled' : !schedule.eventId ? 'not_admitted' : known ? 'observed' : 'unresolved';
  let changed = false;
  const monitor = !terminal && known && (schedule.action.kind === 'message_session' || started);
  if (schedule.eventId && ['failed', 'ambiguous'].includes(schedule.state) &&
      schedule.nextAttemptAt === null && (retry || terminal || monitor)) {
    ...
    changed = sql.exec(
      ...
      now, retry ? 1 : 0, monitor ? 1 : 0,
      retry ? now : monitor ? now + scheduleLimits(env).retryBaseMs : null,
      retry ? 1 : 0, terminal ? 1 : 0, now, started ? 1 : 0, now,
      execution.taskId, execution.sessionId, execution.error, projectId, id, version
    ).rowsWritten > 0;
  }
  const result = getSchedule(sql, projectId, id)!;
  return {
    schedule: { ...result, execution: { ...execution, retrySubmissionAllowed: changed ? false : execution.retrySubmissionAllowed } },
    changed,
    idempotent: !changed,
    actionAlreadyAdmitted: Boolean(schedule.eventId),
    recovery: {
      outcome,
      message: retry ? 'Bounded task submission retry scheduled with the original identities and deadline'
        : known ? 'Canonical execution receipt observed'
        : !schedule.eventId ? 'Schedule has not admitted an action'
