...
export async function processExpiredAttentionMarkers(
  ...
  for (const marker of attention.getExpiredMarkers(sql)) {
    try {
      ...
      if ((marker.kind !== 'needs_input' && marker.kind !== 'reconciliation_checkin') || !marker.taskId) continue;
      ...
      if (marker.workspaceId && env.DATABASE) {
        await idleCleanup.stopWorkspaceInD1(env.DATABASE, marker.workspaceId);
      }
