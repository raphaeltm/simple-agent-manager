...
function checkpoint(sql: SqlStorage, subscriptionId: string): CatchupRow | null {
  return (
    sql
      .exec<CatchupRow>(
        `SELECT id AS subscription_id, channel_id,
    ...
    FROM project_event_subscriptions WHERE id = ? AND channel_id IS NOT NULL`,
        subscriptionId
      )
      .toArray()[0] ?? null
  );
...
export function followChannel(
  ...
  const idempotencyKey = normalizeText(
    ...
    'idempotencyKey',
    resolveProjectEventLimits(env).maxFilterStringBytes
  );
  ...
  const result = createProjectEventSubscription(sql, env, storedProjectId, {
    ...
    expiresAt:
      input.expiresAt ??
      existing?.expiresAt ??
      input.defaultExpiresAt ??
      now + channelLimits(env).cursorTtlMs,
  });
  ...
  if (state) {
    ...
    if (state.channel_id !== row.id)
      throw new ProjectEventCursorError('Channel handoff generation is unavailable');
  } else {
    // An existing unrelated subscription must never gain historical access.
    if (result.idempotent)
      throw new ProjectEventCursorError('Channel handoff checkpoint is unavailable');
    ...
    state = {
      ...
      watermark: row.lifetime_count,
      expires_at: Math.min(
        cursor?.expires ?? now + channelLimits(env).cursorTtlMs,
        result.subscription.expiresAt ?? Number.MAX_SAFE_INTEGER
      ),
      start_fingerprint: fingerprint,
    };
    sql.exec(
      `UPDATE project_event_subscriptions SET channel_id = ?, channel_after_sequence = ?,
      channel_watermark = ?, channel_catchup_expires_at = ?, channel_start_fingerprint = ? WHERE id = ?`,
      state.channel_id,
      state.after_sequence,
      state.watermark,
      state.expires_at,
      state.start_fingerprint,
      state.subscription_id
    );
  }
  if (state.expires_at <= now)
    throw new ProjectEventCursorError('Channel handoff checkpoint expired');
  return {
    ...result,
    watermark: state.watermark,
    caughtUpThrough: state.after_sequence,
...
export function catchUpChannel(
  ...
  assertProjectBinding(storedProjectId, input.projectId);
  const now = Date.now();
  const subscription = readVisibleSubscription(
    sql,
    input.projectId,
  ...
  if (!state || state.expires_at <= now)
    throw new ProjectEventCursorError('Channel handoff checkpoint expired or unavailable');
  const channelRow = sql
    .exec<{
      name: string;
    }>(
      'SELECT name FROM project_event_channels WHERE project_id = ? AND id = ?',
      input.projectId,
      state.channel_id
    )
    .toArray()[0];
  ...
  if (historyHasGap(page, state.after_sequence, state.watermark, hasMore)) {
    throw new ProjectEventCursorError(
      'Channel history retention gap; start a new history snapshot'
    );
  }
  for (const { event } of page) {
    ...
    if (!existing) {
      const count = sql
        .exec(
          `SELECT id FROM project_event_matches WHERE project_id = ? AND event_id = ? LIMIT ?`,
          input.projectId,
          event.id,
          limits.maxMatchesPerEvent
        )
        .toArray().length;
      if (count >= limits.maxMatchesPerEvent)
        throw new ProjectEventLimitExceededError(
          'Channel historical event fanout capacity exceeded'
        );
      insertMatchIfAbsent(sql, event, subscription, now);
    }
  }
  const after = hasMore ? (page.at(-1)?.sequence ?? state.after_sequence) : state.watermark;
  sql.exec(
    'UPDATE project_event_subscriptions SET channel_after_sequence = ? WHERE id = ?',
    after,
    subscription.id
  );
  return {
    subscription,
    idempotent: false,
    changed: after !== state.after_sequence,
    watermark: state.watermark,
    caughtUpThrough: after,
    hasMore,
  };
}
