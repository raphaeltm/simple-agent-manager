...
export const DEFAULT_PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_RECHECK_MS = 60 * 60 * 1000;
...
export function resolveStorageSafetyConfig(env: Env): StorageSafetyConfig {
  ...
  return {
    ...
    toolPayloadCleanupEnabled:
      (env.PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_ENABLED ?? '').trim().toLowerCase() === 'true',
