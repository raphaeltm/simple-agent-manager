// FILE SIZE EXCEPTION: ProjectData is the DO RPC surface; feature logic lives in focused modules and only typed delegation remains here.
/**
          ...
          try {
            const taskRow = this.env.DATABASE
              ? await this.env.DATABASE.prepare(
                ...
                ).bind(workspaceId).first<{ task_mode: string | null }>()
              : null;
    ...
    await idleCleanup.processExpiredCleanups(this.sql, this.env,
      ...
      async (workspaceId, projectId) => {
        ...
        try {
          ...
          ).bind(workspaceId, projectId).first<{ node_id: string | null; user_id: string }>();
          if (wsRow?.node_id) {
