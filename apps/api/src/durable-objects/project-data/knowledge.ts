...
function getMaxObservations(env: Env): number {
  return (
    Number(env.KNOWLEDGE_MAX_OBSERVATIONS_PER_ENTITY) || KNOWLEDGE_DEFAULTS.maxObservationsPerEntity
  );
}
...
export function listEntities(
  ...
  const countRow = sql
    .exec(`SELECT COUNT(*) as cnt FROM knowledge_entities e ${where}`, ...params)
    .toArray()[0];
...
export function addObservation(
  ...
  const obsCount = parseCountCnt(
    sql
      .exec(
        'SELECT COUNT(*) as cnt FROM knowledge_observations WHERE entity_id = ? AND is_active = 1',
        entityId
      )
      .toArray()[0],
    'knowledge_obs_count'
...
export function createRelation(
  ...
  const source = sql
    .exec('SELECT id FROM knowledge_entities WHERE id = ?', sourceEntityId)
    .toArray()[0];
  if (!source) throw new Error(`Source entity not found: ${sourceEntityId}`);
  const target = sql
    .exec('SELECT id FROM knowledge_entities WHERE id = ?', targetEntityId)
    .toArray()[0];
...
export function flagContradiction(
  ...
  const existingRows = sql
    .exec('SELECT * FROM knowledge_observations WHERE id = ?', existingObservationId)
    .toArray();
...
function syncObservationToFts(sql: SqlStorage, observationId: string, content: string) {
  try {
    // Get the rowid
    const rows = sql
      .exec('SELECT rowid FROM knowledge_observations WHERE id = ?', observationId)
      .toArray();
...
function removeObservationFromFts(sql: SqlStorage, observationId: string) {
  try {
    const rows = sql
      .exec('SELECT rowid, content FROM knowledge_observations WHERE id = ?', observationId)
      .toArray();
    if (rows.length === 0) return;
