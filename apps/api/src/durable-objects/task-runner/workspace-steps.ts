...
async function recoverWorkspaceAllocationBySession(
  ...
  const existingWorkspace = await rc.env.DATABASE.prepare(
    ...
    .first<
      CapacityPlacementSnapshotRow & { workspaceId: string; nodeId: string | null }
    >();
...
async function claimWorkspaceAllocationForTask(
  state: TaskRunnerState,
  rc: TaskRunnerContext,
  workspaceId: string,
  now: string
...
async function tombstoneUndispatchedWorkspaceAllocation(
  ...
  await rc.env.DATABASE.prepare(
    ...
    .bind(`Workspace allocation abandoned before dispatch: ${reason}`, now, workspaceId, state.projectId, state.userId)
    .run();
