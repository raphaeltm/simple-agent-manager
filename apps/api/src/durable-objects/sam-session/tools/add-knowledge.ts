  ...
  input_schema: {
    ...
    properties: {
      ...
      entityName: {
        type: 'string',
        description:
          'The name of the knowledge entity (e.g., "User", "Architecture", "CodeStyle").',
      },
      entityType: {
        type: 'string',
        enum: [...KNOWLEDGE_ENTITY_TYPES],
        description:
          'The type of entity (preference, style, context, expertise, workflow, personality, custom).',
      },
      observations: {
        type: 'array',
        items: { type: 'string' },
        description:
          'Array of observation strings to add to the entity. Each is a fact or preference.',
      },
      sourceType: {
        type: 'string',
      ...
      confidence: {
        type: 'number',
        description:
          'Confidence score (0.0 to 1.0). Defaults to 0.9 for explicit, 0.7 for inferred.',
      },
    },
...
export async function addKnowledge(
  ...
  if (input.observations.length > MAX_OBSERVATIONS_PER_CALL) {
    return {
      error: `observations array may not exceed ${MAX_OBSERVATIONS_PER_CALL} items per call.`,
    };
  }
  ...
  const sourceType =
    input.sourceType &&
    KNOWLEDGE_SOURCE_TYPES.includes(input.sourceType as (typeof KNOWLEDGE_SOURCE_TYPES)[number])
      ? input.sourceType
      : 'explicit';
