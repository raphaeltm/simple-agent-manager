...
function getMaxBytes(env: Env): number {
  return env.ATTACHMENT_UPLOAD_MAX_BYTES
    ? parseInt(env.ATTACHMENT_UPLOAD_MAX_BYTES, 10)
    : ATTACHMENT_DEFAULTS.UPLOAD_MAX_BYTES;
}

function getBatchMaxBytes(env: Env): number {
  return env.ATTACHMENT_UPLOAD_BATCH_MAX_BYTES
    ? parseInt(env.ATTACHMENT_UPLOAD_BATCH_MAX_BYTES, 10)
    : ATTACHMENT_DEFAULTS.UPLOAD_BATCH_MAX_BYTES;
}

function getMaxFiles(env: Env): number {
  return env.ATTACHMENT_MAX_FILES
    ? parseInt(env.ATTACHMENT_MAX_FILES, 10)
    : ATTACHMENT_DEFAULTS.MAX_FILES;
}

function getPresignExpiry(env: Env): number {
  return env.ATTACHMENT_PRESIGN_EXPIRY_SECONDS
    ? parseInt(env.ATTACHMENT_PRESIGN_EXPIRY_SECONDS, 10)
    : ATTACHMENT_DEFAULTS.PRESIGN_EXPIRY_SECONDS;
}

// ---------------------------------------------------------------------------
// S3 Client (lazy singleton per request — Workers are single-threaded)
// ---------------------------------------------------------------------------
