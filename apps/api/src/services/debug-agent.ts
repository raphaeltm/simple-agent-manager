...
// Mirrors the Completion/ToolCall interfaces above field-for-field — every
// field the model's chat-completion response can populate is already read
// through optional chaining downstream (completion.choices?.[0]?.message,
...
// defaults as a JSON syntax error — see the call site.
  ...
  try {
    if (call.function.name === 'get_vm_incident') {
      ...
      try {
        ...
        // A syntactically valid but wrongly-shaped payload (e.g. non-string
        // search) falls through to the same bounded `{}` defaults as a JSON
        // syntax error, instead of the previous blind cast handing a
        // mistyped value to queryCloudflareLogs.
      } catch {
        /* bounded defaults */
      }
