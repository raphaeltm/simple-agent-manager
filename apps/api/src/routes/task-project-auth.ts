...
export function requireProjectTaskRead(db: ProjectAuthDb, projectId: string, userId: string) {
  return requireProjectCapability(db, projectId, userId, 'task:read');
}

export function requireProjectTaskWrite(db: ProjectAuthDb, projectId: string, userId: string) {
