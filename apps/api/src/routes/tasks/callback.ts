/**
 * Task status callback route — called by the VM agent with a callback JWT.
 *
 * This route MUST be mounted BEFORE projectsRoutes in the main app to avoid
 * Hono middleware leak. projectsRoutes.use('/*', requireAuth()) would otherwise
 * reject the VM agent's Bearer JWT before this handler's verifyCallbackToken runs.
 *
...
import { canTransitionTaskStatus, getAllowedTaskTransitions, isTaskStatus } from '../../services/task-status';
import { computeBlockedForTask, setTaskStatus } from './_helpers';

const taskCallbackRoute = new Hono<{ Bindings: Env }>();

/**
 * POST /:projectId/tasks/:taskId/status/callback
 *
 * Called by the VM agent to report task execution progress and terminal status.
 * Uses callback JWT auth (Bearer token), NOT session auth.
 */
