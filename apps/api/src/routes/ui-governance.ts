...
uiGovernanceRoutes.put(
  '/standards/:version',
  requireSuperadmin(),
  jsonValidator(UIStandardUpsertSchema),
  async (c) => {
...
uiGovernanceRoutes.post(
  '/components',
  requireSuperadmin(),
  jsonValidator(ComponentDefinitionCreateSchema),
  async (c) => {
    const payload = c.req.valid('json');
    const service = createUiGovernanceService(c.env.DATABASE);
...
uiGovernanceRoutes.put(
  ...
  requireSuperadmin(),
  jsonValidator(ComponentDefinitionUpdateSchema),
  async (c) => {
    const componentId = c.req.param('componentId');
    const payload = c.req.valid('json');
    const service = createUiGovernanceService(c.env.DATABASE);
...
uiGovernanceRoutes.post(
  '/compliance-runs',
  requireSuperadmin(),
  jsonValidator(ComplianceRunCreateSchema),
  async (c) => {
    const payload = c.req.valid('json');
    const service = createUiGovernanceService(c.env.DATABASE);
...
uiGovernanceRoutes.post(
  '/exceptions',
  requireSuperadmin(),
  jsonValidator(ExceptionRequestCreateSchema),
  async (c) => {
...
uiGovernanceRoutes.post(
  '/migration-items',
  requireSuperadmin(),
  jsonValidator(MigrationWorkItemCreateSchema),
  async (c) => {
    const payload = c.req.valid('json');
    const service = createUiGovernanceService(c.env.DATABASE);
...
uiGovernanceRoutes.patch(
  '/migration-items/:itemId',
  requireSuperadmin(),
  jsonValidator(MigrationWorkItemPatchSchema),
  async (c) => {
