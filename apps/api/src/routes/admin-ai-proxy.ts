...
adminAIProxyRoutes.patch('/config', async (c) => {
  ...
  if (body.billingMode === 'unified' && !resolveUnifiedBillingToken(c.env)) {
    throw errors.badRequest(
      'Cannot set billing mode to "unified" without a Cloudflare token configured. '
      + 'Set CF_AIG_TOKEN (preferred) or CF_API_TOKEN as a Worker secret first, or use "auto" mode.',
    );
  }
