...
import { getWorkspacePortsOnNode } from '../../services/node-agent';
...
crudRoutes.post(
  ...
  async (c) => {
    ...
    const placement = await resolveManualWorkspacePlacement({
      db,
      env: c.env,
      userId,
      ...
      activeNodeCount: userNodeCountVal,
      maxNodesPerUser: limits.maxNodesPerUser,
      heartbeatStaleAfterSeconds: limits.nodeHeartbeatStaleSeconds,
      now,
    });
    const targetNodeId = placement.nodeId;
    const mustProvisionNode = placement.mustProvisionNode;
    const credentialAttributionSource = placement.credentialAttributionSource;
    const placementExplanation = placement.explanation;
    ...
    await db.insert(schema.workspaces).values({
      ...
      vmSize,
      vmLocation,
      placementExplanationJson: JSON.stringify(placementExplanation),
      createdAt: now,
      updatedAt: now,
    });
    ...
    log.info('workspace.manual_placement_decided', {
      workspaceId,
      taskId: chatTaskId,
    ...
    c.executionCtx.waitUntil(
      completeManualWorkspacePlacement({
        ...
        vmSize,
        vmLocation,
        gitUserName: auth.user.name,
        gitUserEmail: auth.user.email,
      })
    );
