...
It applies with full force when the helper is named after a _condition_ (`isContainerUnavailableError`)
but every existing caller uses it to decide an _action_ (attempt a recovery). The name reads
...
- The justification is about the _new_ call site only, and the PR body never names the others.
...
5. **A sentinel is coarser than the evidence that justified it.** `errors.Is` compares identity
   ...
   it silently extends the filter to states nobody measured (a snapshot racing a _restart_ is
...
- **Build the error from its real producer AND through the real wrap.** A literal
  ...
  that _re-implements_ the production wrap is the same bug one level up: it keeps passing if the
