...
The tell is a transaction that _binds the new owner_ and _un-binds the old one_ —
...
the transaction is atomic. The defect is an _absence_: no record that this record was
...
1. **The supersession must be recorded on the record being replaced** — a status
   ...
   Record it in the **same transaction** that transfers ownership _by default_. There is
   ...
   _derive_ supersession from lineage, provided that:
...
3. **A reaper must be able to distinguish "superseded" from "dead" — and its verdict
   ...
   requirement-1 exception, but only if it is exact: a _heuristic_ (nearest timestamp,
...
4. **Check the terminal-state predicates before choosing the marker.** Marking the
   predecessor terminal is the obvious move and is often _wrong_: here every
