...
4. **The error handler must not be able to throw.** Persisting a failure record must be
   guarded — a failure while _recording_ a failure must not abort the handler and
...
5. **Never build an idleness/staleness predicate on a column a keepalive path writes.**
   ...
   liveness — pick a different signal (last _work_ activity, a dedicated
...
ceiling is anchored on "the last real progress edge" but the reporter tracks a _set_
...
item you are asking about. Either track progress per item, or make the _reporter_
