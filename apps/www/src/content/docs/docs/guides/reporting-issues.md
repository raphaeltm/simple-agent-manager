...
**Report** stays in the rail after a session stops or fails, so you can still report a session that already ended — which is usually when you want to. For the rest of the rail, and for switching it between icons, labels, and hidden, see [The session tool rail](/docs/guides/chat-features/#the-session-tool-rail).
...
| `PLATFORM_FEEDBACK_INCIDENT_STALE_SINGLETON_MAX_AGE_MS`        | `259200000`  | Max age for one-off pending incidents with no recurrence                                                                                      |
| `PLATFORM_FEEDBACK_INCIDENT_STALE_SINGLETON_EXPIRY_BATCH_SIZE` | `25`         | Max stale singleton incidents expired per sweep                                                                                               |
...
| `PLATFORM_FEEDBACK_INCIDENT_MAX_DISPATCHES_PER_TRIGGER_WINDOW` | `1`          | Max dispatches one incident trigger may submit per rate window                                                                                |
...
| `DEBUG_AGENT_DAILY_TOKEN_LIMIT`   | `480000`              | Daily budget, counted **per feature** — manual diagnosis and automated triage each get this much |
