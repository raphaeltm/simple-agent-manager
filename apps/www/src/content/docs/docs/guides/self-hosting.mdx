...
4. Builds and uploads VM Agent binaries only when those inputs changed or compatibility was explicitly advanced
...
The workflow fails closed if it cannot prove the prior release or publish a required
change. `VM_AGENT_REQUIRED_VERSION` and `VM_AGENT_BUILD_FINGERPRINT` are generated
