...
landing *during* a request is visible to that request's later queries — that is the ordinary
...
Go to Actions → Deploy Production → Run workflow. Choose the `main` branch. You may leave `target_commit_sha` empty to deploy the current `main` tip, or paste an exact 40-character commit SHA if you need to target a specific commit. No additional commit is needed after the environment is configured.
...
3. Choose the `main` branch and leave `target_commit_sha` empty (it defaults to the current `main` tip). Optionally paste a specific 40-character SHA if needed. Leave `dry_run` disabled.
