...
- **Account-link election** (new signups): with signup approval enabled, every new user starts as an ordinary pending candidate. When Better Auth links a usable login account, D1 atomically promotes exactly one candidate to `superadmin`/`active`; concurrent later candidates remain pending. An interrupted signup that never links an account does not consume the election.
