...
These values are stored encrypted in SAM's platform configuration store and can be updated later by a superadmin without redeploying. Environment secrets remain useful as fallback values and for infrastructure credentials that must exist before the app can boot.
