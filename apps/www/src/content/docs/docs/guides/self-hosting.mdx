...
The GitHub App needs **Pull requests: Read-only** for pull request and review webhooks, **Checks: Read-only** for `check_run`/`check_suite`, and **Actions: Read-only** for `workflow_run`. Subscribe it to `push`, `pull_request`, `issue_comment`, `repository`, `check_run`, `check_suite`, `workflow_run`, `pull_request_review`, and `pull_request_review_comment`. Enable **Redirect on update** under the Setup URL settings so repository access changes return users to SAM.
