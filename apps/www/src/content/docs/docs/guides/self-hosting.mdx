...
Fresh deployments are seeded with an internal sentinel user (`system_anonymous_trials`, `status='system'`, created by a database migration) used for anonymous trials. This sentinel is **not** a real user and never counts toward "is this the first human" checks. Three mechanisms guarantee a real human is promoted regardless of the sentinel:

- **Account-link election** (new signups): with signup approval enabled, every new user starts as an ordinary pending candidate. When Better Auth links a usable login account, D1 atomically consumes a one-time clean-install claim and promotes exactly one candidate to `superadmin`/`active`; concurrent later candidates remain pending. An interrupted signup that never links an account does not consume the claim, and an established installation never reopens it merely because no active superadmin remains.
...
The mechanisms are strictly no-ops once an active superadmin exists, and they never auto-promote a `suspended` or `system` account. The login-time hook excludes the sentinel by both `status='system'` and id; the database election and deploy-time migration use persisted status, so custom sentinel ids remain safe. On managed/multi-user deployments nothing changes.

Notes:

- **Promotion happens regardless of `REQUIRE_APPROVAL`.** Even with open registration, the first human owns the deployment.
- **Concurrent first signup:** if multiple brand-new users complete signup at the same moment, exactly one account-linked candidate becomes superadmin; the others remain pending when approval is enabled.
...
The sentinel user id defaults to `system_anonymous_trials`. If your deployment uses a different sentinel id, set `TRIAL_ANONYMOUS_USER_ID` to that value so login-time self-heal excludes it correctly. The account-link election and deploy-time migration exclude sentinels by their persisted `status='system'` flag (not by id), so they stay correct for custom ids. Most deployments never need to set this.
