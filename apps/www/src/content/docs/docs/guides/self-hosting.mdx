...
Choose a deployment domain for `BASE_DOMAIN`. The simplest option is the Cloudflare zone apex (for example, `example.com`). You can also run multiple independent SAM installations in one zone by giving each installation a different subdomain, such as `dev-a.example.com` and `dev-b.example.com`, while using the same `CF_ZONE_ID` for `example.com`. Each deployment then creates namespaced hostnames such as `app.dev-a.example.com`, `api.dev-a.example.com`, and `ws-*.dev-a.example.com`, and derives distinct Cloudflare resource names from its full `BASE_DOMAIN`.
