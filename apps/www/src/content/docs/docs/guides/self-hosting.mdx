...
building is enabled. The workflow stops if release metadata is unreadable or invalid,
publication fails, or `skip_agent` attempts to bypass a first, changed, or unproven
release. `VM_AGENT_REQUIRED_VERSION` and `VM_AGENT_BUILD_FINGERPRINT` are generated
release metadata; self-hosters should not set or rotate them manually.
