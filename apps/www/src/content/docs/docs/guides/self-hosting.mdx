...
Setting it to `false` means every session provisions a cloud VM. Users need an
active personal cloud provider credential unless an administrator has configured
an enabled platform compute credential that SAM can use as the installation
fallback.
