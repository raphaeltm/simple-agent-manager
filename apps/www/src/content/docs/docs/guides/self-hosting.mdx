...
In Cloudflare Dashboard → `https://dash.cloudflare.com/<account-id>/api-tokens` → Create Custom Token:
...
Create R2 access keys for Pulumi state in the same Cloudflare account:
