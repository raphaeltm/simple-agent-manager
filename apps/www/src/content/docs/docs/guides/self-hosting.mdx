...
Your API token is missing the **Zone → SSL and Certificates → Edit** permission. New nodes use this permission when the API Worker signs the node-generated CSR for VM TLS. Edit the token in Cloudflare and add it.
