...
VM diagnostic evidence, ProjectData archived tool payloads, and immutable terminal-archive recovery bundles use the existing application R2 bucket; there is no separate bucket or manually generated secret to configure. Pulumi creates an independent lifecycle rule only for the private diagnostic prefix, while the deployment sync binds the same bucket as `PROJECT_DATA_ARCHIVE_R2` for non-expiring ProjectData archives and recovery evidence. Optional stack settings are `diagnosticIncidentPrefix` (default `diagnostic-incidents`) and `diagnosticIncidentTtlDays` (default `7`, any positive integer). Keep the diagnostic prefix private and distinct from the reserved application namespaces `agents`, `cli`, `compose-image-artifacts`, `library`, `project-data`, `session-snapshots`, `temp-uploads`, and `tts`; Pulumi rejects overrides whose top-level segment would expire objects owned by one of those features.
