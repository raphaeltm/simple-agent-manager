...
- **Login-time self-heal** (OAuth only): on each GitHub OAuth login, if the signing-in user is the only non-system human and no superadmin exists, they are promoted in a single atomic, idempotent write. A failure here never blocks login.
...
- **Cookie-cache staleness:** the session role is cached for up to 5 minutes. If you were promoted mid-session (e.g. the login-time self-heal fired on a session you already held), the new admin access becomes visible after the cache expires or, faster, after you **log out and log back in**.
