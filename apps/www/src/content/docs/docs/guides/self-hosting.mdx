...
Leaving `CF_CONTAINER_ENABLED` at the `true` the deploy workflow injects lets users start
[Instant sessions](/docs/guides/instant-sessions/) when no project, user, or platform VM
credential can satisfy the work.
Setting it to `false` means work must use cloud VM provisioning and requires one
of the VM credential sources to be available. SAM chooses a project-scoped compute
credential first when one is attached to the project, then falls back to the
user's personal compute credential, then to an enabled platform compute credential
...
legacy preset examples, but pool editing is based on concrete provider instance types.
