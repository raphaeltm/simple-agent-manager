...
Each deploy enables D1 read replication (`read_replication.mode = "auto"`) on the main and
observability databases, and the API Worker runs each request against a single D1 session so its
...
sees data older than its own start, and queries that do not open a session always go to the
primary. Scheduled sweeps and Durable Objects keep the unsessioned binding. Set
...
**Cloudflare Workers: D1 — Edit** grant the deploy token already requires.
...
legacy preset examples, but pool editing is based on concrete provider instance types. Exact native
offerings carry their own storage/image/architecture request details where the provider API supports
them; changing a deprecated legacy size label does not change the native provider payload. After a VM
