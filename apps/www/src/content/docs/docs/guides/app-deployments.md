...
- The provider must support first-class block volumes. **Hetzner, Scaleway, Vultr, Infomaniak, and UpCloud are supported; GCP deployment volumes are not** ( is  for the GCP provider), so a GCP-backed environment cannot use persistent deployment volumes. Vultr uses high-performance NVMe Block Storage, which is only offered in a subset of Vultr regions, so a Vultr-backed environment with volumes is pinned to a Block-Storage-capable region.
