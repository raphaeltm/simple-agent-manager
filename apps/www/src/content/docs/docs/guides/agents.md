...
| `search_messages`                   | Search messages by keyword — sessions are indexed incrementally as they sleep or stop, so FTS5 covers everything indexed so far; only messages written since the last index pass fall back to keyword matching. In very large projects `rootSearch` / `coverageNotes` report when only the newest matches were searched |
