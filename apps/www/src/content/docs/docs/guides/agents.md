...
| `search_messages`                   | Search current and archived messages by keyword. Project-wide results may be provisional: repeat the same query, roles, and limit with `archiveSearch.continuation` until `archiveSearch.complete` is true. Owner, index, and execution coverage are reported separately. |
