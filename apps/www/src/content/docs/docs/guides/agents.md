...
| `search_messages`                   | Search current and archived messages by keyword. Sessions are indexed incrementally as they sleep or stop. Project-wide results may be provisional: repeat the same query, roles, and limit with `archiveSearch.continuation` until `archiveSearch.complete` is true; owner, index, and execution coverage are reported separately. |
