...
- **Availability.** Guided sign-in is available on the hosted platform and on self-hosted deployments running on Cloudflare Containers (which SAM requires). If the button isn't shown, use the manual API key or token fields in the same panel.
