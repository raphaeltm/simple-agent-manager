...
| `SESSION_SLEEP_AFTER_MS`                        | `900000` (15 min)         | ProjectData-recorded idle interval before SAM automatically sleeps a VM session. Runtime heartbeats do not extend this clock. Completed tasks queue sleep immediately; a terminal prompt still marked active becomes eligible once this interval has elapsed. Ledger cleanup also uses this interval from task completion to protect the final response.                                                                                                                                                     |
