...
| `SESSION_SLEEP_MAX_ATTEMPTS`                         | `9`                       | Automatic sleep attempts before SAM preserves compute and records an operator-visible failure. A task failure starts a fresh budget; a failed task's preservation stops at it even for repairable captures, then tears the runtime down and says so in the chat. Raising the budget re-arms previously exhausted rows that are still below the new limit.                                                         |
| `FAILED_TASK_PRESERVATION_MAX_WAIT_MS`               | `28800000` (8 hours)      | Longest a failed task's runtime stays awake waiting for its preservation sleep, from the failure. A turn that never ends defers that sleep without spending an attempt; past this wait the sweep tears the runtime down and says in the chat that the work was not saved (`releaseStalledFailedTaskPreservation()` in `apps/api/src/services/failed-task-preservation-release.ts`).                               |
