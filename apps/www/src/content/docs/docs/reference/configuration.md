...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_PROJECT_IDS`                  | _(empty)_                       | Optional comma-separated project allowlist for automatic cleanup; empty preserves cleanup eligibility for every project, while an emergency rollout can scope elevated budgets to exact projects    |
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_CUTOFF_CREATED_AT`            | _(empty)_                       | Optional fixed exclusive tool-message cutoff; when set, the immutable verified manifest, cumulative ceilings, plan ID, and exactly one allowlisted project are required; invalid values fail closed |
...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_BATCH_BYTES`                  | `2097152`                       | Hard maximum legacy `tool_metadata` bytes read into JS by one automatic cleanup pass; a fixed exact plan is invalid unless this is at least the per-row ceiling                                     |
...
| `PROJECT_DATA_TOOL_PAYLOAD_MANUAL_CLEANUP_MAX_BATCH_BYTES`       | `2097152`                       | Hard legacy `tool_metadata` byte cap for one explicit manual cleanup pass                                                                                                                           |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_BATCHES`              | `100`                           | Overall slice ceiling after which an incomplete preflight terminates as truncated                                                                                                                   |
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_ROWS`                 | `500000`                        | Overall matching-row ceiling for one preflight plan                                                                                                                                                 |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_WALL_TIME_MS`             | `20000`                         | Per-slice ProjectData RPC timeout; timeouts retain the cursor and fail closed for retry                                                                                                             |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_RUN_WALL_TIME_MS`         | `25000`                         | Aggregate scheduled-invocation wall-time ceiling; must exceed the per-slice wall-time budget by the return margin                                                                                   |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_STATE_BYTES`          | `1750000`                       | D1 JSON byte ceiling independently enforced for accumulated session and target-batch proof state                                                                                                    |
...
| `PROJECT_DATA_ARCHIVE_GLOBAL_SWEEP_INTERVAL_MS`                  | `86400000`                      | Persisted cadence gate for unscoped scheduled archive-sharding sweeps; daily by default even though the Worker scheduled handler wakes every five minutes                                           |
...
1. Inspect current telemetry with `GET /api/admin/project-data/storage?projectId=<projectId>`.
2. Measure bounded reclaim candidates with `POST /api/admin/project-data/storage/:projectId/relief-measure`; this is the dry-run/measurement path and remains cursor-resumable and bounded.
3. If tool-payload relief is needed, run one explicit manual slice with `POST /api/admin/project-data/storage/:projectId/tool-payload-cleanup` and a non-empty `reason` plus unique `idempotencyKey`. Optional `batchRows`, `batchBytes`, and `wallTimeMs` are capped by the manual hard-max env vars above.
4. Inspect the response `terminationReason`, before/after `databaseSize`, reclaimed bytes, row counts, cursor/recheck state, and project cooldown. Retrying the same idempotency key replays a stored completed result; if a crash leaves the key in progress, the same key is held during cooldown and can retry after cooldown. A different key during cooldown is skipped.
5. Re-measure storage with `POST /api/admin/project-data/storage/:projectId/measure` before any scoped archive-sharding dry-run/canary under the separate rollout controls.
