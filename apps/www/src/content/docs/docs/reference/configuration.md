...
| `PROJECT_DATA_ARCHIVE_GLOBAL_SWEEP_INTERVAL_MS`                  | `86400000`                      | Persisted cadence gate for unscoped scheduled archive-sharding sweeps; the five-minute handler claims the cadence row only when due. Code fallback is daily; the checked-in `wrangler.toml` ships `900000` (15 min), see the load review below    |
...
| `PROJECT_DATA_ARCHIVE_SWEEP_SESSIONS`                            | `10`                            | Hard ceiling on sessions (in-flight plus new candidates) one archive-sharding pass may process                                                                                                                                                    |
...
| `PROJECT_DATA_ARCHIVE_WALL_TIME_MS`                              | `5000`                          | Soft wall-clock budget for one archive-sharding cron pass, checked between candidates (one large session may run past it). Code fallback 5 s; the checked-in `wrangler.toml` ships `30000` so a tick drains several small tail sessions           |
...
| `PROJECT_EVENT_SOURCE_OUTBOX_BATCH_ROWS`                | `25`              | Maximum producer-side event admission intents retried per trigger cleanup pass                                                                                                   |
...
| `PROJECT_EVENT_WAKE_ENABLED`                            | `true`            | Enables same-chat ProjectData event wake materialization for v2 `existing_session_prompt` subscriptions; set `false` to leave event delivery pull-only                           |
...
| `CREDENTIAL_LIMIT_ADMISSION_MAX_ACTIVE_PER_PROJECT`     | `1000`            | Maximum retained credential-limit event admission envelopes per project                                                                                                          |
| `CREDENTIAL_LIMIT_ADMISSION_RETRY_BATCH_SIZE`           | `25`              | Maximum pending or failed credential-limit admission envelopes retried per bounded maintenance pass                                                                              |
| `CREDENTIAL_LIMIT_ADMISSION_MAX_ATTEMPTS`               | `6`               | Maximum ProjectData dispatch attempts for one credential-limit admission envelope                                                                                                |
| `CREDENTIAL_LIMIT_ADMISSION_RETRY_DELAY_MS`             | `60000`           | Delay before retrying a failed credential-limit admission dispatch                                                                                                               |
| `CREDENTIAL_LIMIT_ADMISSION_RETENTION_DAYS`             | `30`              | Retention window for credential-limit window state and legacy admission rows                                                                                                     |
