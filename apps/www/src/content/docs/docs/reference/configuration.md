...
| `TRIAL_ANONYMOUS_USER_ID`                | `system_anonymous_trials`            | Id of the internal anonymous-trial sentinel user used by login-time self-heal. Database election and backfill exclude every `status='system'` row. Override only if your deployment uses a different sentinel id.                                                                                                            |
