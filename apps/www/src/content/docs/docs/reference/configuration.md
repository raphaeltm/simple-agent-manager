...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_CUTOFF_CREATED_AT`         | _(empty)_                       | Optional fixed exclusive tool-message creation cutoff in epoch milliseconds; malformed or future values fail closed so a scoped rollout cannot silently widen its approved row set               |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_BATCH_ROWS`            | `5000`                          | Maximum matching rows examined by one preflight slice                                                                                                                                            |
...
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_MAX_BYTES`             | `2000000000`                    | Overall eligible metadata-byte evidence ceiling for one preflight plan                                                                                                                           |
| `PROJECT_DATA_STORAGE_RELIEF_PREFLIGHT_LEASE_MS`              | `60000`                         | D1 claim lease that prevents overlapping scheduled slices                                                                                                                                        |
