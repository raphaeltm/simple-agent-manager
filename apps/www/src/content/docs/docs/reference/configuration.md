...
| `SESSION_SNAPSHOT_TOTAL_BUDGET_BYTES`       | `268435456` (256 MiB)     | Max combined size of the home + work-in-progress snapshot. The default favors cheaper retained R2 state over keeping a VM alive when real harness state exceeds 100 MiB.                                                                                                                                                       |
| `SESSION_SNAPSHOT_ENTRY_THRESHOLD_BYTES`    | `52428800` (50 MB)        | Largest single file the snapshot scanner will include                                                                                                                                                                                                                                                                        |
...
| `SESSION_SLEEP_AFTER_MS`                    | `900000` (15 min)         | ProjectData-recorded idle interval before SAM automatically sleeps a VM session. Runtime heartbeats do not extend this clock. Completed tasks queue sleep immediately and wait only for the completing prompt to report idle.                                                                                                  |
| `SESSION_SLEEP_SWEEP_BATCH_SIZE`            | `10`                      | Maximum due VM sleeps atomically claimed by one scheduled sweep.                                                                                                                                                                                                                                                             |
| `SESSION_SLEEP_RETRY_DELAY_MS`              | `300000` (5 min)          | Retry delay after a fail-closed automatic sleep attempt.                                                                                                                                                                                                                                                                     |
| `SESSION_SLEEP_MAX_ATTEMPTS`                | `3`                       | Automatic sleep attempts before SAM preserves compute and records an operator-visible failure.                                                                                                                                                                                                                               |
