...
| `PROJECT_EVENT_WAKE_ENABLED`                                | `false`            | Enables same-chat ProjectData event wake materialization for v2 `existing_session_prompt` subscriptions; opt in with `true`; unset or `false` leaves event delivery pull-only                                              |
