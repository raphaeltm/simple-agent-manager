...
| `SESSION_SLEEP_AFTER_MS`                             | `900000` (15 min)         | ProjectData-recorded idle interval before SAM automatically sleeps a VM session. Runtime heartbeats do not extend this clock. Completed and failed tasks queue sleep immediately (a failed task's sleep preserves its unpushed work); a terminal prompt still marked active becomes eligible after this interval from the later activity or task-completion timestamp. Ledger cleanup also uses this interval from task completion or failure to protect the final response.                |
