...
| `PROJECT_EVENT_SOURCE_OUTBOX_BATCH_ROWS`                | `25`               | Maximum producer-side outbox row mutations per trigger cleanup pass; ProjectData admission starts only when two mutations are available for claim plus settlement                |
