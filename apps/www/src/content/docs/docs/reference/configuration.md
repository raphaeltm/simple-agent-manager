...
| `VM_AGENT_REQUIRED_VERSION`                    | _(deploy-generated)_ | Required vm-agent build for reusable VM nodes. Official deploys derive this from the last commit that changed `packages/vm-agent`, not from the deployment commit, so a Worker-only deploy does not make every running node ineligible for reuse. Binaries are published under that immutable release key first, and cloud-init requests that exact release. Leave unset only for local/manual development or skip-agent deploys. |
