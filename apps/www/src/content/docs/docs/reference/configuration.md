...
| `PROJECT_DATA_ARCHIVE_SWEEP_FALLTHROUGH_DEPTH`                   | `4`                             | Extra candidates read beyond the pass's session slots, so a candidate the daily write budget refuses can descend to a smaller one instead of ending the pass with nothing migrated                                                                 |
...
| `PROJECT_DATA_ARCHIVE_DAILY_WRITE_BUDGET` | `250000` | Installation-wide daily estimated SQL write allowance for compact migration attempts; 0 pauses admission. Also the source of the per-candidate selection ceiling, so lowering it automatically narrows which sessions a sweep will select. The checked-in `wrangler.toml` ships `800000`. |
| `PROJECT_DATA_ARCHIVE_WRITE_ESTIMATE_FACTOR` | `32` | Conservative SQL write estimate per row/512 bytes of grouped FTS text; includes source deletion. The checked-in `wrangler.toml` ships `8` (measured amplification 4.6-6.7x). |
