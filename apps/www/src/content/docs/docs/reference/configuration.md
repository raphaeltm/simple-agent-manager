...
| `VM_AGENT_REQUIRED_VERSION`                    | _(deploy-generated)_ | Required vm-agent build for reusable VM nodes. Official deploys derive this from the last commit that changed a vm-agent **build input**, not from the deployment commit, so a Worker-only deploy does not make every running node ineligible for reuse. Build inputs are `packages/vm-agent/**` excluding `.claude/`, `AGENTS.md` and `*_test.go`, so a docs, rule or test-only commit under that directory deliberately leaves the release — and the node pool — untouched. Binaries are published under that immutable release key first, and cloud-init requests that exact release. Leave unset only for local/manual development or skip-agent deploys. |
...
| `PROJECT_DATA_ARCHIVE_SWEEP_SESSIONS`                            | `10`                            | Hard ceiling on sessions (in-flight plus new candidates) one archive-sharding pass may process; the checked-in `wrangler.toml` ships `1` after the 2026-09-08 billing firebreak                                                                          |
