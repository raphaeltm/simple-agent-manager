...
| `VM_INCIDENT_RECONCILE_BATCH_SIZE`    | `50`                   | Maximum artifacts/incidents repaired per scheduled pass (minimum: 5) |
...
| `TASK_LIVENESS_PROBE_TIMEOUT_MS`                   | `5000` (5 sec)                         | Per-candidate timeout for the ACP liveness probe and the Instant (cf-container) lifecycle-inspection probe used by ProjectData heartbeat deferral and stuck-task reconciliation; a timeout is inconclusive (never fails a task or terminalizes a session)              |
