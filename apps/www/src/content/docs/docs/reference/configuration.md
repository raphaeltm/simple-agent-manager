...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_CUTOFF_CREATED_AT`         | _(empty)_                       | Optional fixed exclusive tool-message cutoff; when set, an immutable plan ID and exactly one allowlisted target project are required, and malformed/future values fail closed                    |
