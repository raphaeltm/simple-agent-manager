...
| `PROJECT_DATA_ARCHIVE_SHARDING_ENABLED`                    | `false`                         | Exact opt-in for the external terminal transcript archive coordinator. Missing, malformed, and non-`true` values disable all migration work                                            |
| `PROJECT_DATA_ARCHIVE_MAX_CANDIDATES_PER_SWEEP`            | `1`                             | Maximum terminal archive journals claimed by one scheduled Worker sweep                                                                                                                |
| `PROJECT_DATA_ARCHIVE_MAX_CANDIDATES_HARD_LIMIT`           | `25`                            | Configurable safety ceiling applied to the per-sweep candidate limit                                                                                                                   |
| `PROJECT_DATA_ARCHIVE_MAX_CHUNKS_PER_SWEEP`                | `8`                             | Maximum independently idempotent transcript chunks copied by one sweep                                                                                                                 |
| `PROJECT_DATA_ARCHIVE_MAX_CHUNKS_HARD_LIMIT`               | `100`                           | Configurable safety ceiling applied to transcript and manifest chunks processed per sweep                                                                                              |
| `PROJECT_DATA_ARCHIVE_CHUNK_MAX_ROWS`                      | `128`                           | Maximum rows in one source-to-target archive RPC chunk                                                                                                                                 |
| `PROJECT_DATA_ARCHIVE_CHUNK_MAX_ROWS_HARD_LIMIT`           | `1000`                          | Configurable safety ceiling applied to rows in one archive RPC                                                                                                                         |
| `PROJECT_DATA_ARCHIVE_CHUNK_MAX_BYTES`                     | `8388608`                       | Maximum canonical bytes in one archive RPC chunk; runtime also caps this below the 32 MiB Durable Object RPC ceiling                                                                   |
| `PROJECT_DATA_ARCHIVE_LEASE_MS`                            | `300000`                        | D1 migration journal lease duration; each reclaim increments the fence epoch                                                                                                           |
| `PROJECT_DATA_ARCHIVE_TERMINAL_GRACE_MS`                   | `604800000`                     | Minimum age after terminalization before a session can become an archive candidate                                                                                                     |
| `PROJECT_DATA_ARCHIVE_RETRY_BASE_MS`                       | `300000`                        | Initial retry delay for a failed archive journal                                                                                                                                       |
| `PROJECT_DATA_ARCHIVE_RETRY_MAX_MS`                        | `86400000`                      | Maximum exponential retry delay for a failed archive journal                                                                                                                           |
| `PROJECT_DATA_ARCHIVE_CIRCUIT_FAILURES`                    | `3`                             | Consecutive project migration failures before its archive circuit opens                                                                                                                |
| `PROJECT_DATA_ARCHIVE_CIRCUIT_OPEN_MS`                     | `3600000`                       | Project archive circuit-open duration                                                                                                                                                  |
| `PROJECT_DATA_ARCHIVE_SHARD_COUNT`                         | `16`                            | Deterministic packed archive-owner buckets per project                                                                                                                                 |
| `PROJECT_DATA_ARCHIVE_SHARD_COUNT_HARD_LIMIT`              | `256`                           | Configurable safety ceiling applied to packed archive-owner buckets                                                                                                                    |
| `PROJECT_DATA_ARCHIVE_SEARCH_MAX_OWNERS`                   | `7`                             | Maximum archive owners queried with the root search plane; results report partial coverage when exceeded                                                                               |
| `PROJECT_DATA_ARCHIVE_SEARCH_MAX_OWNERS_HARD_LIMIT`        | `7`                             | Non-raiseable safety ceiling that keeps project-wide read aggregation within the request I/O budget                                                                                    |
| `PROJECT_DATA_ARCHIVE_SWEEP_MAX_WALL_MS`                   | `20000`                         | Wall-clock budget for one external coordinator sweep; the protocol ceiling is 30000 ms                                                                                                 |
| `PROJECT_DATA_ARCHIVE_SWEEP_MAX_IO_OPS`                    | `30`                            | Conservative external-operation reservations per sweep; the non-raiseable protocol ceiling is 30 and work yields before an indivisible stage cannot fit                                |
| `PROJECT_DATA_ARCHIVE_COPY_EXPANSION_RATIO`                | `2`                             | Canonical-byte expansion multiplier reserved during root/archive rehome capacity planning; actual target `databaseSize` is rechecked before source deletion                            |
| `PROJECT_DATA_ARCHIVE_ERROR_MAX_CHARS`                     | `2000`                          | Maximum retained journal error characters; the protocol ceiling is 10000                                                                                                               |
| `PROJECT_DATA_ARCHIVE_R2_PREFIX`                           | `project-data/archive-sharding` | Non-expiring private `project-data/...` prefix for immutable canonical recovery chunks and linked sealed-manifest pages; other namespaces fail closed                                  |
| `PROJECT_DATA_ARCHIVE_ROOT_COPY_MAX_RATIO`                 | `0.7`                           | Maximum root `databaseSize` ratio allowed by explicit copy-back; an unsafe root request requires one explicit configured non-root fallback owner and never scans owners synchronously  |
| `PROJECT_DATA_ARCHIVE_ROUTING_VERSION`                     | `1`                             | Worker routing contract version checked by the pre-deploy D1 compatibility guard                                                                                                       |
...
| `PROJECT_DATA_TOOL_PAYLOAD_CLEANUP_MAX_ROW_BYTES`          | `1048576`                       | Legacy inline-size threshold for cleanup accounting; archival may read larger rows up to `PROJECT_DATA_TOOL_PAYLOAD_ARCHIVE_MAX_METADATA_BYTES`                                        |
...
| `SESSION_INDEX_MAX_ROWS`                                   | `1000`                          | Per-sync keyset page budget for the D1 `session_summaries` index. Coverage stays incomplete until bounded pages converge, then switches to delta sync                                  |
...
Terminal archive placement is not driven by a ProjectData alarm. The external
scheduled Worker owns the D1 journal/lease/fence and is disabled by default.
Exact reads fail closed while a pointer is transitional; archive owners remain
immutable after publication. Rollback freezes and forward-fixes a journal using
its private R2 recovery bundle—operators must not delete source payloads or
change authoritative location rows manually.
