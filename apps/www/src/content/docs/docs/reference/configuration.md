...
| `PROJECT_DATA_ARCHIVE_SWEEP_SESSIONS`                            | `10`                            | Hard ceiling on sessions (in-flight plus new candidates) one archive-sharding pass may FENCE and process; a candidate the daily write budget refuses opens no fence and consumes no slot; the checked-in `wrangler.toml` ships `2`                 |
