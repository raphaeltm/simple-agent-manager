...
Reserved cookies comprise all `sam_*` names; `COOKIE_NAME` (default `vm_session`); exact
workspace-scoped `${COOKIE_NAME}_${workspaceULID}` names; and the exact BetterAuth session
names `better-auth.session_token`, `__Secure-better-auth.session_token`, and
`__Host-better-auth.session_token`. Similarly named extension cookies are preserved as
application-owned.

Privileged terminal, ACP, boot-log, and log-stream WebSockets use exact configured
control-plane origins. Configuration containing `*` fails validation, and runtime matchers
also never honor wildcard entries. Preview sibling origins are rejected before authentication
or state mutation, and no-origin non-browser clients retain their existing token-authenticated
flow.
...
This CLI-only HTTP route forwards a local service through an authenticated workspace request.
It does not accept WebSocket upgrades and is separate from exposed-port preview routing.
...
| `COOKIE_NAME`                      | `vm_session`                                               | VM session cookie base name. The Worker setting `VM_AGENT_COOKIE_NAME` propagates this value to new cloud VMs and Instant containers. Preview isolation always reserves this name and the legacy default.                                                                                                                             |
| `ALLOWED_ORIGINS`                  | Exact API and app origins derived from `CONTROL_PLANE_URL` | Comma-separated exact trusted origins for privileged WebSockets. Entries containing `*` fail validation; no-origin token-authenticated clients remain supported.                                                                                                                                                                      |
