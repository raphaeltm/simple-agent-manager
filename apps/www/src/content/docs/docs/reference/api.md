...
Agents can retrieve durable project event-bus data with `get_event`, `list_subscription_events`, and `ack_event_delivery`. `get_event` returns one visible event by stable event ID, including normalized metadata and payload. `list_subscription_events` paginates deliveries visible through an authorized active/unexpired subscription with opaque subscription-bound cursors; list summaries include metadata and delivery state but not payloads. `ack_event_delivery` idempotently returns success for already-acknowledged deliveries, acknowledges `queued` or `delivered` deliveries only when the independent delivery policy requires acknowledgement, and rejects non-ack, `failed`, or `expired` deliveries. The server derives project/task/session/workspace identity from the verified MCP token and current D1 project predicates, so callers cannot pass identity fields or distinguish nonexistent records from unauthorized ones.
