...
A **provider** is the cloud your compute runs on. For running workspaces, SAM supports **Hetzner** and **Scaleway**. SAM follows a **Bring Your Own Cloud (BYOC)** model: you add your own provider credential in **Settings → Cloud Provider** (or **Connections**), and VM costs are billed directly to your account. On the hosted platform, compute can be provided for you, so you may not need your own credential. Cloud credentials are never stored as environment variables.
...
<Aside type="note">
  Google Cloud is supported as a **deployment target** for shipping your app (see [App
  Deployments](/docs/guides/app-deployments/)), not as a provider for running workspaces.
</Aside>
