...
Which one you get is an [agent profile](#agent-profiles) setting: a profile whose runtime is **Instant container** starts chats on Instant; everything else runs as a task on a VM. Instant needs no cloud provider credential, so it remains available for container-suitable profiles and as a fallback when no project, user, or platform VM credential can satisfy the work.
