...
Infrastructure Compute Pools add a lower-level control surface for teams that need it: project,
user, and installation pools are reconciled from the full provider-native catalog for the available
credentials, then edited as concrete instance offerings rather than only small/medium/large presets.
Default pools start with only the concrete offerings that match those legacy presets selected; other
provider-native offerings stay catalog-only until you add them.
