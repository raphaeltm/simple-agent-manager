...
A **VM workspace** gives the agent a full Linux toolchain, Docker, and your `.devcontainer`, and has:
...
- Its own lifecycle: `pending` → `creating` → `running` → `stopping` → `stopped`, plus `recovery` and `sleeping` for [Instant sessions](/docs/guides/instant-sessions/)

An **Instant** workspace trades those away for a start measured in seconds — see the comparison below.
...
When you **start a chat**, SAM picks for you: **connect a cloud provider credential and new chats run on your VMs; connect nothing and they run Instant.** An [agent profile](#agent-profiles) can pin a runtime explicitly. Submitted tasks always use a VM.

Instant sessions sleep when idle and are restored from a snapshot when they wake or when their container is reclaimed. See [Instant Sessions](/docs/guides/instant-sessions/).
