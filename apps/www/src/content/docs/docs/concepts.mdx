...
Every workspace has one or more agent chat sessions and its own lifecycle: `pending` → `creating` → `running` → `stopping` → `stopped`, plus `sleeping` and `recovery` on the Instant runtime.
