...
:::note
On a hosted instance, cloud compute is usually provided for you, so you don't need your own cloud provider credential. If you're self-hosting or the operator requires it, add a [Hetzner API token](https://console.hetzner.cloud/), [Scaleway secret key](https://console.scaleway.com/iam/api-keys), or [Vultr API key](https://my.vultr.com/settings/#settingsapi), or [Infomaniak application credential](https://docs.infomaniak.cloud/identity/applications_credentials/) under **Settings → Cloud Provider**.
:::
