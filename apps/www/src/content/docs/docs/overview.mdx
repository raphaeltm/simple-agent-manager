...
<CardGrid>
  ...
  <Card title="Ephemeral by Design" icon="refresh">
    Workspaces spin up in minutes and tear down when you're done. Warm node pooling reuses idle VMs for fast follow-up tasks.
  </Card>
...
6. **Submit a task** — describe what you want done; SAM provisions a VM, creates a workspace, and runs the agent autonomously
7. **Chat with agents** — watch real-time streaming output, fork conversations, use voice input
...
- **Task execution** — autonomous agent coding with warm node pooling for fast reuse
...
- **In-app notifications** — task progress, agent requests, PR creation alerts
