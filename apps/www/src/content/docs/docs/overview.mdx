...
8. **Review the result** — in the standard task workflow the agent commits, pushes its branch, and opens a pull request when done; the workspace is ephemeral, so nothing is persisted unless the agent pushes
