...
SAM supports **Bring-Your-Own-Cloud (BYOC)**: users and self-hosters may store their own Hetzner, Scaleway, Vultr, Infomaniak, DigitalOcean, or GCP credentials, encrypted per-user in D1. This is the model for self-hosted deployments and BYO-key users.
...
| ------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------ |
| Cloud provider credentials      | VM provisioning (Hetzner, Scaleway, Vultr, Infomaniak, DigitalOcean, GCP WIF or service-account JSON) | AES-256-GCM, per-credential IV |
