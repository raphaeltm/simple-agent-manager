...
D1 also stores a producer-side `project_event_source_outbox` for source facts that must survive a failed ProjectData admission call. It stores bounded canonical event input, retry state, expiry, and final admission outcome. The established task-terminal transition helper inserts the lifecycle intent in the same D1 batch as the authoritative task transition; other current lifecycle hook callers enqueue at their hook boundary and are documented as narrower guarantees rather than blanket lifecycle capture.
