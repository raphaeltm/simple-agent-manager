...
const legacySelectedHetznerSkus = {
  cx23: 'small',
  cx33: 'medium',
  cx43: 'large',
} as const;
...
function nativeOffering(index: number) {
  if (index === 35) return nativeOfferings[2]!;
...
function capacityCandidate(scope: PoolScope, index: number) {
  ...
  const provider =
    index < visibleLocations.length || index === 5 || index === 35
      ? 'hetzner'
      : providerNames[index % providerNames.length];
  const machineSize =
    provider === 'hetzner'
      ? (legacySelectedHetznerSkus[offering.sku as keyof typeof legacySelectedHetznerSkus] ?? null)
      : null;
  return {
    ...
    status: machineSize ? 'active' : 'disabled',
    createdAt: TIMESTAMP,
    updatedAt: TIMESTAMP,
  };
}
...
async function removeAshHilCandidates(
  page: Page,
  editHeading: string,
  screenshotName: string,
  scope: PoolScope
) {
  const initialSummary = capacitySummary(scope);
  const expectedAllowedAfterSave = initialSummary.activeCandidateCount - 1;
  const expectedExcludedAfterSave = initialSummary.candidates.length + 1 - expectedAllowedAfterSave;
  ...
  await page.getByRole('button', { name: /Remove Hetzner hel1 cx43/ }).click();
  ...
  await page.getByLabel('Maximum monthly price').fill('1500');
  ...
  await screenshotSectionNearHeading(
    page,
    'Catalog filters',
    `${screenshotName}-catalog-expensive-not-selected`
  );
  ...
  await expect(
    page.getByText(
      new RegExp(
        `${expectedAllowedAfterSave} allowed · ${expectedExcludedAfterSave} not selected/removed`
      )
    )
  ).toBeVisible();
  await expect(
