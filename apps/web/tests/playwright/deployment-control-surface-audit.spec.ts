...
test.describe('Deployment control surface audit', () => {
  ...
  test('configuration panel shows variables and masked secrets without overflow', async ({ page }) => {
    await setupMocks(page);
    await page.goto(`/projects/${PROJECT_ID}/deployments`);
    ...
    await expect(stagingCard.getByText(/staging\.deploy-audit-with-a-very-long-hostname/)).toBeVisible();
    await expect(stagingCard.getByText('DATABASE_URL')).toBeVisible();
    await expect(stagingCard.getByText('Hidden after save')).toBeVisible();
    await expect(stagingCard.getByPlaceholder('DATABASE_URL')).toBeVisible();

    await screenshot(page, 'deployment-config-panel');
    await assertNoOverflow(page);
  });
});
