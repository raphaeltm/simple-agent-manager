...
async function setup(page: Page, diagnosisStatus = 200) {
  await page.route('**/api/**', async (route: Route) => {
    ...
    if (path === '/api/admin/observability/diagnoses' && request.method() === 'GET') return respond(200, { diagnoses: [] });
    if (path === '/api/admin/observability/diagnoses' && request.method() === 'POST') {
      return diagnosisStatus === 200 ? respond(200, { diagnosis: DIAGNOSIS }) : respond(diagnosisStatus, { error: 'BUDGET_EXHAUSTED', message: 'Daily deployment debugging budget exhausted' });
    }
