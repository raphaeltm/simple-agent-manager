...
async function setupApiMocks(page: Page) {
  await page.route('**/api/**', async (route: Route) => {
    ...
    if (path === '/api/credentials') return respond(200, [{ id: 'cred-1', provider: 'hetzner', name: 'Hetzner' }]);
...
test.describe('Fork/retry new chat screen audit', () => {
  test('fork returns to the new chat screen with lineage banner and settings controls', async ({ page }, testInfo) => {
    ...
    await expect(page.getByLabel(/Workspace:?|Workspace profile/)).toBeVisible();

    const textareaValue = await page.getByPlaceholder('Describe what you want the agent to do...').inputValue();
  ...
  test('retry returns to the new chat screen with original task text and error context', async ({ page }, testInfo) => {
    await setupApiMocks(page);
    await page.goto('/projects/proj-test-1/chat/session-1');
