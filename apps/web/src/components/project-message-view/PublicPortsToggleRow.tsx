    ...
    <div className="border-t border-[rgba(34,197,94,0.08)] px-4 py-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          {error && (
            <p className="mt-1 flex items-center gap-1 text-[11px] leading-snug text-red-300" role="alert">
        ...
        <button
          ...
          onClick={onToggle}
          disabled={saving}
          className="relative inline-flex h-11 w-[72px] shrink-0 items-center rounded-full border border-[rgba(148,163,184,0.28)] bg-surface-hover px-1 transition-colors disabled:cursor-wait disabled:opacity-70 data-[checked=true]:bg-[rgba(34,197,94,0.28)]"
          data-checked={enabled}
        >
          <span className={`block h-8 w-8 rounded-full bg-fg-primary shadow transition-transform ${enabled ? 'translate-x-7' : 'translate-x-0'}`} />
        </button>
      </div>
    </div>
  );
