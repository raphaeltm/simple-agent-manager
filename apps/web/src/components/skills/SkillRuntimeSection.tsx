...
export const SkillRuntimeSection: FC<SkillRuntimeSectionProps> = ({
  projectId,
  skillId,
}) => {
  ...
  const loadConfig = useCallback(async () => {
    try {
      ...
      const res = await getSkillRuntimeConfig(projectId, skillId);
      setConfig(res);
      loadedRef.current = true;
    } catch {
      toast.error('Failed to load skill runtime config');
    } finally {
      setLoading(false);
    }
  }, [projectId, skillId, toast]);
  ...
  const handleAddEnvVar = async () => {
    ...
    setSaving(true);
    try {
      const res = await upsertSkillRuntimeEnvVar(projectId, skillId, {
        key,
        value: envValue,
        isSecret: envSecret,
      });
  ...
  const handleDeleteEnvVar = async (key: string) => {
    setSaving(true);
    try {
      const res = await deleteSkillRuntimeEnvVar(projectId, skillId, key);
      setConfig(res);
  ...
  const handleAddFile = async () => {
    ...
    setSaving(true);
    try {
      const res = await upsertSkillRuntimeFile(projectId, skillId, {
        path,
        content: fileContent,
        isSecret: fileSecret,
      });
  ...
  const handleDeleteFile = async (path: string) => {
    setSaving(true);
    try {
      const res = await deleteSkillRuntimeFile(projectId, skillId, path);
      setConfig(res);
