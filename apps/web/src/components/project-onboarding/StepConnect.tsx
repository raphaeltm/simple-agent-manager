    ...
    <div className="grid gap-4">
      ...
      <div className="grid gap-4 rounded-md border border-border-default bg-surface p-4">
        {isArtifacts ? (
          <Callout variant="info">
            SAM will host this repository on Cloudflare Artifacts and seed it with a README that orients
            your agents. No GitHub account or app installation is required.
          </Callout>
          ...
          <>
            ...
            <label htmlFor="project-onboarding-repository" className="grid gap-1.5">
              ...
              {fieldErrors.repository && (
                <span id="project-onboarding-repository-error" className="text-sm text-danger" role="alert">
                  {fieldErrors.repository}
                </span>
              )}
              {fieldErrors.githubRepoId && (
                <span id="project-onboarding-repo-id-error" className="text-sm text-danger" role="alert">
                  {fieldErrors.githubRepoId}
                </span>
              )}
            </label>
      ...
      {isArtifacts ? (
        <WhyDetails question="Where does the code live, and how do agents push?">
          <p>
            SAM provisions a private Git repository on Cloudflare Artifacts and stores its remote on the
            project. Each agent run mints a short-lived, repository-scoped token to clone and push — the
            token can only touch this one repo.
          </p>
          <p>
            There are no pull requests on a SAM-hosted repo: agents push their branch straight to the
            remote and you review the changes there. The default branch starts as <code>main</code>.
          </p>
        </WhyDetails>
      ) : (
        <WhyDetails question="Why does SAM need a GitHub App, and how is access scoped?">
          <p>
            SAM reaches your code through a GitHub App installation, not your personal token. Whenever an
            agent runs, SAM mints a short-lived token scoped to just this repository — so the App’s
            permissions are the hard ceiling on what any agent can ever do here.
          </p>
          <p>
            The branch you pick is the base agents branch off of. Each run works on its own branch, so SAM
            never pushes to your default branch on its own. A finished task agent auto-pushes that branch
            and opens a PR for review; a conversation agent only pushes when you ask it to.
          </p>
        </WhyDetails>
      )}
    </div>
