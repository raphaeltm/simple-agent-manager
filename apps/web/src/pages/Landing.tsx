      ...
      <Container maxWidth="sm">
        <div className="text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 pt-6">
            {[
              { label: 'Cloud VMs', sub: 'Powered by Hetzner', color: 'text-info' },
              { label: 'Claude Code', sub: 'Pre-installed', color: 'text-success' },
              { label: 'Pay as you go', sub: 'Stop when done', color: 'text-purple' },
            ].map((item) => (
              <div key={item.label} className="text-center p-3 bg-surface rounded-md border border-border-default">
                <div className={`text-[length:var(--sam-type-section-heading-size)] font-[number:var(--sam-type-section-heading-weight)] ${item.color}`}>{item.label}</div>
