...
func TestGetAgentCommandInfo_OAuthToken(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantEnvVar:        "",
			wantInstallCmd:    "npm install -g @agentclientprotocol/codex-acp@1.1.2 @openai/codex@0.144.5",
...
func TestGetAgentCommandInfoOpenAICodex(t *testing.T) {
	...
	if info.installCmd != "npm install -g @agentclientprotocol/codex-acp@1.1.2 @openai/codex@0.144.5" {
		t.Fatalf("installCmd=%q, unexpected", info.installCmd)
	}
