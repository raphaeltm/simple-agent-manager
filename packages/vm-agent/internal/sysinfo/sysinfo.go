...
type DockerInfo struct {
	...
	ContainerList []ContainerStats `json:"containerList"`
}
...
type CollectorConfig struct {
	DockerTimeout  time.Duration // Timeout for Docker CLI commands (default: 10s)
...
func NewCollector(cfg CollectorConfig) *Collector {
	if cfg.DockerTimeout == 0 {
		cfg.DockerTimeout = 10 * time.Second
	}
...
// collectDocker queries Docker CLI for version and container stats.
func (c *Collector) collectDocker() DockerInfo {
	...
	// Get container stats
	ctx2, cancel2 := context.WithTimeout(context.Background(), c.config.DockerTimeout)
	defer cancel2()
	out, err = exec.CommandContext(ctx2, "docker", "stats", "--no-stream", "--format",
		`{"id":"{{.ID}}","name":"{{.Name}}","image":"{{.Image}}","status":"running","cpuPercent":"{{.CPUPerc}}","memUsage":"{{.MemUsage}}","memPercent":"{{.MemPerc}}"}`).Output()
	if err == nil {
		...
		for _, line := range lines {
			...
			cs.CPUPercent = parsePercentString(raw["cpuPercent"])
			cs.MemPercent = parsePercentString(raw["memPercent"])
			info.ContainerList = append(info.ContainerList, cs)
		}
	}

	info.Containers = len(info.ContainerList)
	return info
}
