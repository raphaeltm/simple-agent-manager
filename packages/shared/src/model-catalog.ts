...
type ModelEntry = readonly [id: string, name: string];

function modelGroup(label: string, models: readonly ModelEntry[]): ModelGroup {
  return {
    label,
    models: models.map(([id, name]) => ({ id, name, group: label })),
  };
}
