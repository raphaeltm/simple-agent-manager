...
export function createApiClient(baseUrl = ''): ApiClient {
  return {
    ...
    loadSource: async (target: string, source: SourceRef, sourceIndex: number, options = {}) => {
      const payload = await postJson<{ preview: SourceReadResult }>(
        `${baseUrl}/api/source-preview`,
        {
          ...
          sourceIndex,
          path: source.path,
          ...(source.lineGroups ? { lineGroups: source.lineGroups } : {}),
          ...(options.fullFile ? { fullFile: true } : {}),
        }
      );
    ...
    acceptThread: (threadId: string, input: ThreadAcceptInput = {}) =>
      postJson<ThreadAcceptResult>(
        `${baseUrl}/api/threads/${encodeURIComponent(threadId)}/accepted`,
        input
      ),
  };
}
