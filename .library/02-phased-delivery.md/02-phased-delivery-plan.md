...
- SAM accepts Compose YAML as the primary release authoring format, parses it into the normalized manifest, and renders the node-side Compose file server-side from that manifest. Raw manifest JSON remains accepted for backward compatibility.
...
- **Agent deploy tools (the primary interface):** `get_registry_credentials(environment)` → scoped push token; `submit_deployment(environment, composeYaml)` → SAM parses the Compose subset, validates the normalized manifest, resolves tags to digests, renders, signs, applies; `get_deployment_status/logs(environment)` read-only. The agent never touches the node — it pushes bytes to the registry and *proposes* a Compose-authored release. Trust boundary unchanged.
...
- Compose-submission path extends beyond the slice-2 single-service limit: multi-service manifests, multiple named volumes, and the single pre-flight release hook for migrations (doc 10 §5).
