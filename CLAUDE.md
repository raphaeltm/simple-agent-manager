...
- **ProjectData DO**: Each project has a root Durable Object, accessed via `env.PROJECT_DATA.idFromName(projectId)`, for lifecycle metadata, active transcripts, activity events, ACP sessions (spec 027), and canonical `project_event_*` event-subscription storage. The disabled-by-default external terminal-archive coordinator may place immutable terminal transcript payloads in exact packed owners named `project-data-archive:<projectId>:<shard>`; D1 owner/generation routing is authoritative and transitional or ambiguous reads fail closed.
