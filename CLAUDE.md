...
- durable-instant-launch-and-sweep-like-fix: Instant (cf-container) launches now run durably in the TaskRunner DO (`durable-objects/task-runner/instant-launch.ts`): both launch paths (`chat-start.ts`, MCP `dispatch-instant.ts`) accept inline then hand the continuation to `startInstantLaunch` (job + alarm committed transactionally) instead of request-scoped `ctx.waitUntil`, which Cloudflare cancels ~30s post-response without running catch blocks (2026-08-07 stuck-queued incident). Interrupted attempts are classified by durable milestone: fail-closed full teardown (`markInstantLaunchFailed`) before `bootstrap_complete`, idempotent `finalizeInstantLaunch` after. Also fixed the `stuck_tasks` sweep, dead since 2026-08-06: its mismatch-dedup bound a 52-byte LIKE pattern against D1's 50-byte `SQLITE_LIMIT_LIKE_PATTERN_LENGTH` (row-evaluated, so empty-table tests passed) — now two ≤50-byte patterns, per-candidate evaluation isolation (rule 53), and sweep recovery fails the linked chat session. Process rule `.claude/rules/55-d1-statement-limits-and-request-scoped-waituntil.md`.
