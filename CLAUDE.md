...
- dependency-governance-hardening: Four non-trivial Dependabot bumps integrated and verified as one batch. `@agentclientprotocol/sdk` was **removed** from `packages/acp-client` rather than bumped to 1.3.0 — it had zero importers; ACP payload shapes are hand-rolled in `src/hooks/useAcpMessagePayloads.ts` and `src/transport/types.ts`, `src/transport/websocket.ts` types the payload as `unknown`, and the canonical wire contract lives in the Go VM agent. The false claim in `packages/acp-client/AGENTS.md` that the SDK "defines the ACP wire protocol types" was corrected. `Dockerfile.vm-agent-container` moved to `node:26-bookworm-slim`; `Dockerfile.sandbox` to `cloudflare/sandbox:0.12.5`, with `@cloudflare/sandbox` now **exact-pinned** so the npm client cannot drift from the digest-pinned image. `apps/www` moved to `astro@7` (required by `@astrojs/starlight@0.41.7`'s `astro@^7.0.2` peer); Astro 7's rolldown pipeline resolves dynamic imports inside processed `<script>` blocks, so the mermaid lazy-loader in `BlogPost.astro` needed `is:inline`. New `scripts/quality/dependency-governance.test.ts` checks close the gap that let two of these ship half-done: every `FROM` (including workflow-embedded heredoc Dockerfiles) must be preceded by a matching reviewed-source-tag comment, and every `@cloudflare/sandbox` declaration in any workspace must be an exact pin equal to the image tag. Note: Dependabot cannot group across ecosystems, so an image bump and its npm SDK bump still arrive as separate PRs — the governance test is the tripwire that forces them to be reconciled.
- report-issue-idea-flow: Hosted "Report an Issue" flow that creates draft Ideas in a configurable feedback project (`PLATFORM_FEEDBACK_PROJECT_ID`). Two entry points: SessionHeader action row (expanded) and ErrorBoundary crash screen. Users explicitly consent before technical refs (sessionId, taskId, nodeId) are attached. Server-side cross-tenant ref authorization validates project membership before storing references. User text sanitized with secret/PII redaction and fenced with provenance markers. Feature auto-hidden when `PLATFORM_FEEDBACK_PROJECT_ID` is unset or does not reference an existing project in the current deployment database. Configurable limits: `REPORT_ISSUE_TITLE_MAX_LENGTH`, `REPORT_ISSUE_DESCRIPTION_MAX_LENGTH`, `REPORT_ISSUE_CONTENT_MAX_LENGTH`.
