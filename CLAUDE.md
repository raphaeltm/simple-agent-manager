...
- dependency-batch-2026-08: Nine stale Dependabot PRs landed as one batch. `@agentclientprotocol/sdk` was **removed** from `packages/acp-client` rather than bumped to 1.3.0 — it had zero importers; ACP payload shapes are hand-rolled in `src/hooks/useAcpMessagePayloads.ts` and `src/transport/types.ts`, `src/transport/websocket.ts` types the payload as `unknown`, and the canonical wire contract lives in the Go VM agent (`packages/vm-agent/internal/acp/`). The false claim in `packages/acp-client/AGENTS.md` that the SDK "defines the ACP wire protocol types" was corrected. `Dockerfile.vm-agent-container` moved to `node:26-bookworm-slim` (Node 26 is *Current*; it becomes LTS 2026-10-28. Taken over Active-LTS 24 because 24 enters maintenance 2026-10-20 and 26 is supported to 2029-04-30 — and, decisively, because the Instant container path is now verifiable end-to-end on staging, so undici 8's `fetch` rewrite was closed by a real agent turn instead of by preference). `Dockerfile.sandbox` moved to `cloudflare/sandbox:0.12.7` with `@cloudflare/sandbox` **exact-pinned** to the same version so the npm client cannot drift from the digest-pinned image. `apps/www` moved to `astro@7` (required by `@astrojs/starlight@0.41.7`'s `astro@^7.0.2` peer); Astro 7's rolldown pipeline resolves dynamic imports inside processed `<script>` blocks, so the mermaid lazy-loader in `BlogPost.astro` needed `is:inline`. Also `wrangler` 4.123.0, `react`/`react-dom` 19.2.8, `@types/react` 19.2.18, `@commitlint/*` 21.2.2, `modernc.org/sqlite` 1.56.0, `anthropics/claude-code-action` v1.0.193. New `scripts/quality/dependency-governance.test.ts` closes the gap that let several of these ship half-done: Dependabot bumps one manifest at a time and cannot group across ecosystems, so a paired artifact routinely arrives with only one half moved — the image digest without its npm SDK, `react` without `react-dom`, `@commitlint/config-conventional` without `@commitlint/cli`, a `FROM` without its reviewed-source-tag comment. The test is the tripwire that forces reconciliation; the lockstep pairs are also recorded as comments in `pnpm-workspace.yaml`.
