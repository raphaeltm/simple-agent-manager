...
## Recent Changes
- devcontainer-named-volumes: Replaced host-clone + bind-mount devcontainer storage with named Docker volumes (`sam-ws-<workspaceId>`); clone-into-volume via throwaway `alpine/git` container; `devcontainer up --mount source=<vol>,target=/workspaces,type=volume` overrides default bind-mount; eliminated permission normalization dance (`ensureWorkspaceWritablePreDevcontainer`, `ensureWorkspaceWritable`, `getContainerUserIDs`, `getContainerCurrentUserIDs` removed); workspace deletion now removes container and volume; `config.DeriveRepoDirName` exported for cross-package use
