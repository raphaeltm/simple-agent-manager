...
## Recent Changes
- token-message-concatenation: MCP `get_session_messages` now groups consecutive streaming tokens (assistant, tool, thinking roles) into logical messages via `groupTokensIntoMessages()` before returning to agents; `search_messages` description updated to note cross-token search limitation; configurable via MCP_MESSAGE_LIST_LIMIT (default: 50), MCP_MESSAGE_LIST_MAX (default: 200)
