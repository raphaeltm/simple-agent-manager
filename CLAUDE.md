...
- **durable-project-eventing**: Same-chat event prompts share durable mailbox admission and recovery fences. ProjectData owns one-off schedules, bounded standing watches, agent channel history/follow, and member Events controls. D1 source outbox retries lifecycle/GitHub/webhook admission; supported provider-limit telemetry is bound to the actual credential. Schedule admission differs from delivery/agent execution, and ambiguous receipts are never blindly replayed. See `guides/scheduled-actions.md` and the API/reference skills for contracts and limits.
