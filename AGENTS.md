...
| Experimental profile fails to launch                                    | If a model-test or experimental profile fails because the selected model is unavailable or access is missing, mark the profile archived/disabled or switch it to a known-working fallback only with a note to verify availability before reuse. |
