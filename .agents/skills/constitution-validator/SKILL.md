---
name: constitution-validator
description: 'Constitution compliance validator focusing on Principle XI (No Hardcoded Values). Detects hardcoded URLs, timeouts, limits, and identifiers that should be configurable. Use proactively when implementing business logic, adding new features, or reviewing PRs for constitution compliance.'
metadata:
  short-description: 'Constitution compliance validator focusing on Principle XI (No H'
---

# constitution-validator

This is a Codex skill wrapper around the Claude Code subagent definition in:

- `.claude/agents/constitution-validator/CONSTITUTION_VALIDATOR.md`

Use:

1. Read `.claude/agents/constitution-validator/CONSTITUTION_VALIDATOR.md`.
2. Follow its checklist and constraints.
3. Report results with concrete file references.
