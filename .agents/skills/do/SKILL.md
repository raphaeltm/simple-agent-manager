...
8. **PR** — create with `gh pr create`, wait for CI, then comment `@coderabbitai review` once the PR is otherwise good to go. Treat CodeRabbit as merge-blocking: implement or review/close all CodeRabbit feedback, request a fresh CodeRabbit review after fixes, and repeat until the agent and CodeRabbit agree there is no unresolved feedback. If the user requested draft PR / do-not-merge, stop at the draft PR and do not merge.
