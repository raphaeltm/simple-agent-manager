...
| MCP                | 4h sliding / 24h hard cap | `MCP_TOKEN_TTL_SECONDS`, `MCP_TOKEN_MAX_LIFETIME_SECONDS` | Sliding window TTL refreshed on use (throttled to 1 write per 50% TTL); hard 24h max lifetime | Agent MCP tool calls during task execution                                     |
