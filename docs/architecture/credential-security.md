...
Input validation: `agentType` must be one of `claude-code`, `openai-codex`, `google-gemini`, `mistral-vibe`. `credentialKind` must be `api-key` or `oauth-token`. Payload capped at 64 KB.
