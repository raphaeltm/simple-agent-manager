...
describe('D1 Database', () => {
  it('creates database with correct name', async () => {
    ...
    expect(name).toMatch(/^[a-z][a-z0-9-]*-production$/);
  });
});
```
...
# Login to R2 backend
pulumi login 's3://{prefix}-pulumi-state?endpoint=<account-id>.r2.cloudflarestorage.com&region=auto'
```
    ...
    steps:
      ...
      - uses: pulumi/actions@v5
        with:
          ...
          stack-name: prod
          work-dir: infra
          cloud-url: 's3://{prefix}-pulumi-state?endpoint=${{ secrets.CF_ACCOUNT_ID }}.r2.cloudflarestorage.com&region=auto'
        env:
